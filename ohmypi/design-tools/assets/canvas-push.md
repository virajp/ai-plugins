...
Shared by every vwf surface that talks to Claude Design: `/skill:vwf-design-system`
(token sheets, publish, import) and `/skill:vwf-screens` (surface resolution and the
...
never travel through here** — `/skill:vwf-mockups` and blueprint §6a render only into
...
CLAUDE.md (device frame, layout — written by `/skill:vwf-screens`); **two platforms
...
project — `/skill:vwf-design-system` imports *from* it; nothing in vwf pushes to it.)
...
1. Read `design.projects.<registry-project>.<platform>` from `.config/vwf.yaml`.
   ...
   primary-platform pin for every UI project — honor them and nudge `/skill:vwf-setup`
...
3. **Offer to pin** the resolved id under
   ...
   via `/skill:vwf-git-workflow` (`chore(vwf): pin/stamp design project`), riding the
