...
and `/skill:vwf-setup` writes up the extraction as a recommendation. Everything else on
this page is a menu.

Declining it under `enforcement:` is honoured, and honoured honestly: the
finding drops to a warning `/skill:vwf-doctor` reports every run, so nothing halts and
