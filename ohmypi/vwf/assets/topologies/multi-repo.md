...
product-wide mise config and the local stack. `/skill:vwf-setup` runs
...
it could not inspect**. `/skill:vwf-execute` is the exception and
...
`/skill:vwf-doctor` raises a violation as **blocking**, and
`/skill:vwf-setup` writes up the extraction as a recommendation — one
...
`/skill:vwf-setup` from **inside the new repo**: finding no config
...
registry projects are `/skill:vwf-architecture`'s to record.
