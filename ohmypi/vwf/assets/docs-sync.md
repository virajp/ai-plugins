...
2. **CLAUDE.md** — the repo's agent instructions: stack claims, task names,
   layout, the vwf section (`/skill:vwf-setup` owns its shape; this sync only fixes
...
4. **App changelog** (only when the registry's `cross_cutting` accepted the
   ...
   behavior on a device screen platform** (`desktop` / `mobile` / `tablet` /
   `auto`), append a draft entry to the app repo's
...
- **Find contradicted docs graph-first**: when the repo carries a knowledge
  graph, per `%%AI_PLUGINS_ROOT%%/assets/graphify.md` query it for the docs
...
- **Broad drift** (the README no longer resembles the repo): regenerate via
  `/skill:vwf-readme` instead of patching sentence by sentence.
- **Same commit flow**: the sync lands through `/skill:vwf-git-workflow` with the
