...
reality therefore ends by delegating to /skill:vwf-docs-sync, in the
...
/skill:vwf-docs-sync owns it: scope resolution, the
`docs-sync-surveyor` scan (graph-first), surgical edits, the
/skill:vwf-readme route for broad README drift, the change-log hook,
...
`docs:` via /skill:vwf-git-workflow.
