...
3. Compare:
   ...
   - **Stamp behind, only at the legacy location, OR missing while a
     ...
     then offer `/skill:vwf-setup`:
     > "This repo is on blueprint format `<N>`; vwf now ships `<M>`. Run
     > `/skill:vwf-setup` to reconcile the tree to the current format."
...
4. **Halt only if blocking.** Proceed with the requested operation when its
   ...
   format introduced). Never auto-migrate — `/skill:vwf-setup` does that behind its
   own consent gate.
