...
> `/skill:vwf-setup` builds graphs, behind explicit consent.
...
- `docs/memory/` — recall belongs to the memory layer
  (`%%AI_PLUGINS_ROOT%%/assets/memory.md`); a graph copy freezes at the last commit
...
`/skill:vwf-setup` writes the file: the standard set plus whatever
...
`/skill:vwf-doctor` §8 reports a missing one as a **degradation**,
