...
declares, and the facts `/skill:vwf-doctor` checks are supplied by the
...
`/skill:vwf-setup` and `/skill:vwf-execute` both halt on it.
...
(`%%AI_PLUGINS_ROOT%%/assets/stack-adapter.md`). Since config_format 13 the
...
`/skill:vwf-architecture` can match a project's declared capabilities against them:

```yaml
---
axis: backing
name: <display name>
capabilities: [] # from %%AI_PLUGINS_ROOT%%/assets/capability-vocabulary.md
...
`/skill:vwf-plan` and `/skill:vwf-execute` as the `conventions:` field of the
...
`%%AI_PLUGINS_ROOT%%/assets/stack-adapter.md`.
...
`/skill:vwf-architecture` presents every template for a project's `role` and the user
...
`harness` block for `/skill:vwf-doctor` to check — and the pipeline ran on with those
