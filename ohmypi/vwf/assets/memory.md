...
`/skill:vwf-setup` writes the `.gitignore` entries (`docs/memory/handoff/`,
...
> **`/skill:vwf-handoff` and `/skill:vwf-recall` never skip.** The handoff *is* the
...
- **room** — a **closed set of seven**. `decisions` (design/architecture
  ...
  progress, for resuming a paused run; see below), `doctor` (`/skill:vwf-doctor`'s
  ...
  `/skill:vwf-handoff` and `/skill:vwf-recall` own).
  ...
  **Never invent an eighth.** mempalace creates a room implicitly on first
  ...
  silence is why the set is closed and why `/skill:vwf-doctor` checks it.

## Repo config — `mempalace.yaml`

**One product, one wing — and one config file per mined tree, at its root.**
`/skill:vwf-setup` writes them; `/skill:vwf-doctor` reports
...
exists to prevent, and `/skill:vwf-doctor` blocks on it.
...
`/skill:vwf-execute` keeps a single drawer per plan in room `runs` (drawer =
...
resource cap (`/skill:vwf-handoff` → later `/skill:vwf-recall next`), a resumed run reads
