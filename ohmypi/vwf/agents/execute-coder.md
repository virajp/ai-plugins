---
name: execute-coder
description: Code-stage implementer for the /skill:vwf-execute command. Invoked only
  by
  /skill:vwf-execute — do not delegate to it for general tasks. Implements the approved
...
`%%AI_PLUGINS_ROOT%%/assets/memory.md`), read the full findings, and address
...
`%%AI_PLUGINS_ROOT%%/assets/memory.md`, file the full gap to room `gaps` in
...
**Orient graph-first.** Per `%%AI_PLUGINS_ROOT%%/assets/graphify.md`, when a
...
2. **GREEN** — write the **minimum** code to make that test pass, walking the
   decision ladder in `%%AI_PLUGINS_ROOT%%/assets/minimalism.md` first: reuse
