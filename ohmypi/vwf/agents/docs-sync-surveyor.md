...
description: Stateless doc-drift surveyor for the /skill:vwf-docs-sync skill.
  Invoked only by /skill:vwf-docs-sync — do not delegate to it for general tasks.
...
2. **Graph first, files second.** Per
   `%%AI_PLUGINS_ROOT%%/assets/graphify.md`: when a graph exists,
