...
`%%AI_PLUGINS_ROOT%%/assets/docs-sync.md`; this skill is the procedure it names.
...
- **Broad README drift** (the surveyor returns `BROAD DRIFT` instead of
  enumerating): regenerate via /skill:vwf-readme rather than
...
- **Standalone**: commit via /skill:vwf-git-workflow with a `docs:`
  message.
