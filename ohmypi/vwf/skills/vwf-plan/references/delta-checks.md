...
via `/skill:vwf-git-workflow`) and drop the element from the chain.
...
visual render — recommend the user run `/skill:vwf-mockups <flow>` (a local scratchpad
render), or `/skill:vwf-screens import <flow>` when a
