...
description: Produce reviewable cycle plans as diffs for one slice of the
  ...
  back through /skill:vwf-blueprint before writing — so no cycle builds on a gap.
  Requires the blueprint coverage stamp to read complete, and halts on any
  /skill:vwf-doctor blocking finding across the chain's projects.
---
...
complete (`<remaining list, or 'never swept'>`). Run `/skill:vwf-blueprint` to finish
...
`<slice>`. Run `/skill:vwf-blueprint` first." A request that spans **several flows**
...
is behind what vwf ships, nudge `/skill:vwf-setup` (proceed unless a needed artifact
is missing).
...
**Stack gate.** Once the chain is approved, run `/skill:vwf-doctor` scoped to every
...
- **Blocking only.** Unlike `/skill:vwf-execute` this gate does **not** ask about a
...
pause, present the gap, and offer `/skill:vwf-blueprint <flow|entity>` (or
`/skill:vwf-architecture` for a registry hole). After that pass lands (and re-stamps
...
or the user consciously amends the contract via `/skill:vwf-blueprint` (which demotes
...
`/skill:vwf-execute`, closing those plan holes is the point of the pass — fold each
...
write, invoke `/skill:vwf-git-workflow` to ensure an isolated worktree. All git
actions in this command go through /skill:vwf-git-workflow. Keep the worktree **local** —
...
`/skill:vwf-archive` and `/skill:vwf-execute` read to find a
...
- **Approve & execute** (the last element) — commit, then hand into
  `/skill:vwf-execute` **for the first unexecuted plan of the chain** (execute
...
- **Reject** — then either **Revise** (apply feedback, re-present, looping until
  ...
  state its exact path, offer via `/skill:vwf-git-workflow` to remove it and the
...
After each approval, commit that plan via `/skill:vwf-git-workflow`. Use a
