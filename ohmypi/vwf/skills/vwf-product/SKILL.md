...
| Template | `%%AI_PLUGINS_ROOT%%/assets/templates/product.md` |
...
- **Exists → update mode.** Preserve confirmed content. Ask only about genuine
  ...
  `/skill:vwf-feedback` has been logging readings): a metric missing its target is a
...
`%%AI_PLUGINS_ROOT%%/assets/format-check.md`; on drift, **nudge** `/skill:vwf-setup`
...
Elicit following `%%AI_PLUGINS_ROOT%%/assets/elicitation.md` — one decision
