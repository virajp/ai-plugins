...
- Otherwise list the active plans from the base repo's `docs/plans/index.md`
  ...
  cloned (`%%AI_PLUGINS_ROOT%%/assets/membership.md`).
...
Report the moved paths. Commit the move via `/skill:vwf-git-workflow` (a
`docs(plan): archive <slice>` message); all git actions go through
/skill:vwf-git-workflow. Then, if mempalace is available, mark the plan's run journal
