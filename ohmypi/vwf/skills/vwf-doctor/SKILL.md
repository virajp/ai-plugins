...
sits unread, and `/skill:vwf-setup` / `/skill:vwf-execute`
...
- **Never install anything, and never build anything.** Report the command; the
  ...
  long LLM-driven job reserved for `/skill:vwf-setup`.
...
`%%AI_PLUGINS_ROOT%%/assets/membership.md` — a run started inside a member must check
...
onboarded — `/skill:vwf-setup` will onboard it.
...
absent is partway along the chain: `/skill:vwf-setup` writes the
config, and the registry arrives later from
`/skill:vwf-architecture`. Report it as **information** — *early: next
`/skill:vwf-product`, then `/skill:vwf-architecture`* —
...
`/skill:vwf-setup`, and check what you can from its platforms' templates meanwhile.
...
same treatment `/skill:vwf-verify` gives a criterion it already knows is failing. Skip
...
does: the stamp is behind, and `/skill:vwf-setup` will reconcile the
...
Nudge `/skill:vwf-setup` and stop there; a template doctor guessed would silently
...
**Callers.** `/skill:vwf-setup`'s shared spine runs this over the whole
repo, right after it writes the config, and records what
it finds. `/skill:vwf-plan` runs it scoped to its dependency chain's projects, once
the chain is approved and before its survey. `/skill:vwf-execute` runs it scoped to
...
`/skill:vwf-execute` additionally gates on the LSP findings, as it always has;
`/skill:vwf-plan` does not, since it compiles nothing.
