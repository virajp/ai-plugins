...
- **LSP** — the row names a plugin. Check it is active
  ...
  (`%%AI_PLUGINS_ROOT%%/assets/stack-adapter.md`) — never a suggestion to
...
`harness` block to check against — remedy `/skill:vwf-setup`, which
...
`cicd`, without which `/skill:workflow` has to ask on every run. Report each as
drift naming the project and the axis, and nudge `/skill:vwf-architecture` — never
...
`/skill:vwf-setup`, which offers the consent-gated restructure; doctor reports and
...
same finding one level up: report it and nudge `/skill:vwf-setup`, which delegates to
`devtools:scaffold`.
...
→ `10` drift; report and nudge `/skill:vwf-setup`.
