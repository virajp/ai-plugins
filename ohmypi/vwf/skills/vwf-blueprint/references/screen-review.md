...
3. **Review.** The user reviews the rendered screens. Remarks route **now**:
   ...
   `/skill:vwf-design-system`, parked per the elicitation protocol's parked-scope rule
...
4. **Design-first (alternative to 1–3).** The user may prefer their design tool to
   *design* these screens rather than review vwf's contract-derived render: run
   `/skill:vwf-screens prompt <flow>` (it writes the per-platform briefs under
   ...
   `/skill:vwf-screens import <flow>` closes it through a targeted pass here, folding
