...
description: Maintain the always-current, full-product blueprint under
  ...
  /skill:vwf-plan halts without a complete stamp.
---
...
product's** as-of state before anything downstream consumes it — `/skill:vwf-plan`
...
found. Run `/skill:vwf-product` first — the blueprint needs the goals every flow must
...
**Halt if it does not exist:** "No registry found. Run `/skill:vwf-architecture` first
...
flow has UI but no design system. Run `/skill:vwf-design-system` first." Screens
...
**registry** `docs/blueprint/registry.yaml` precisely — via `/skill:vwf-architecture`
...
moved, so the code is no longer known to match; the next `/skill:vwf-plan` for that
...
explicitly skipped, the drop stands and a later `/skill:vwf-mockups <flow>` re-renders
...
to `partial`. This stamp is what `/skill:vwf-plan` gates on.
...
After approval, hand **all** git actions to `/skill:vwf-git-workflow` — it owns
...
continue straight into `/skill:vwf-plan` for the highest-priority slice (from the
