...
**never a gate for `/skill:vwf-plan`**.
...
through `/skill:vwf-blueprint <flow>` or `/skill:vwf-design-system` — then re-run this
...
- No flow folders under `docs/blueprint/flows/` → "No blueprint found. Run
  `/skill:vwf-blueprint` first." Stop.
- No design system (neither file nor folder form) → "Screens reference the
  design system; run `/skill:vwf-design-system` first." Stop.
...
`/skill:vwf-setup` on drift (proceed unless the Screens/design-system artifacts this
command consumes are missing — then tell the user to run `/skill:vwf-setup` and stop).
...
to the repo's `.gitignore` and commit that one line via `/skill:vwf-git-workflow`
...
`/skill:vwf-blueprint <flow>` or `/skill:vwf-design-system`, then re-render.
...
platforms. This is the render-currency state `/skill:vwf-plan`'s soft advisory reads
(and `/skill:vwf-blueprint` drops when a flow's Screens change unrendered).
...
line from §1): hand that to `/skill:vwf-git-workflow` with a
