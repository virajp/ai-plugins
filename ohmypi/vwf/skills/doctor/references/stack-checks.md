...
`harness` block to check against — remedy `/skill:setup`, which
...
drift naming the project and the axis, and nudge `/skill:architecture` — never
...
`/skill:setup`, which offers the consent-gated restructure; doctor reports and
