...
production feedback — it routes exactly like `/skill:vwf-feedback` input. A clean run
...
`/skill:vwf-setup` on drift (proceed unless the needed Acceptance blocks are missing —
a pre-format-4 repo has nothing to verify flows against; tell the user to run
`/skill:vwf-setup` then stop).
...
the **feedback routing** (`/skill:vwf-feedback`'s rules — this command's failures are
just automated feedback):

- **Criterion FAIL** (behavior regression) → file to room `gaps`; offer the fix
  path now — `/skill:vwf-blueprint <flow|entity>` if the blueprint is wrong,
  `/skill:vwf-plan <slice>` for a fix cycle. Deferred → record one line in the owning
...
`/skill:vwf-git-workflow`. Verify is otherwise read-only.
