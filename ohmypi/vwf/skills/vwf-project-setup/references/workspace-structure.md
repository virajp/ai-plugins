...
templates and `/skill:vwf-setup` presents them, exactly as it presents stack
...
| [`polyrepo`](%%AI_PLUGINS_ROOT%%/assets/topologies/polyrepo.md) | A group of repos, wired as submodules under a **parent** repo | the parent repo         |
...
Both are **incremental** — `/skill:vwf-setup` re-runs against the delta rather than
...
**One entry per role is not a default.** `/skill:vwf-architecture` presents the menu
...
`.config/vwf.yaml` and that `/skill:vwf-doctor` checks the repo against; its prose
...
like the project axis, and `/skill:vwf-architecture` presents them **per project**.
