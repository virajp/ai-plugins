...
`%%AI_PLUGINS_ROOT%%/assets/format-check.md` and nudge `/skill:vwf-setup` on drift —
...
secret denylist added — and `/skill:vwf-doctor` raises a duplicate, a misplaced one,
...
submodule of the product parent — which `/skill:vwf-doctor` raises as a **blocking**
finding and `/skill:vwf-setup` offers to restructure. It ships with `config_format` 14,
...
wording is a product decision. `/skill:vwf-screens prompt` transcribes the block into
...
and `/skill:vwf-screens import` match on. Format 10 = format 9 **plus** the
...
- **API contracts** move to `docs/blueprint/apis/<project>.openapi.yaml` — one
  ...
  the frozen production snapshots `/skill:vwf-verify` writes; from the first snapshot
...
`docs/blueprint/product.md` (type **`vwf-product`**, authored by `/skill:vwf-product`)
...
- **`4 → 5`** → author `docs/blueprint/product.md` via `/skill:vwf-product` (elicit —
  ...
  `/skill:vwf-blueprint`** (the reviewer enforces it on touch) — the migration does
...
- **`8 → 9`** → the process-based restructure, in **two phases**:
  ...
  **Phase 2 — elicited fill** (not this migration): the scaffold cannot invent
  ...
  flows that never existed — offer `/skill:vwf-blueprint` (consent-gated); coverage
...
- **`9 → 10`** → **project-grouped, execution-ordered flows**:
  ...
  5. Rewrite flow identifiers in `.config/vwf.yaml`: `design.flows_pushed`
     ...
     old names — the next `/skill:vwf-mockups` sweep or `/skill:vwf-screens` session renames
...
- **`10 → 11`** → **device-grouped flows + pinned screen codes**:
  ...
  3. In-car variants recorded as Screens-row deviations under the format-10
     ...
     pending in-car flows in `remaining:`, and offer `/skill:vwf-blueprint`
...
- **`11 → 12`** → **screen components as contract**: every Screens row gains a
  **Components block** (per the flow template) — the elements the screen
  ...
  by `/skill:vwf-blueprint`** (the reviewer enforces the block on touch) — a missing
  ...
  **not** mechanical and is deferred, on the `9` precedent (coverage stays
  ...
  5. **Queue the density pass — the sweep performs it.** Do not condense during
     ...
     **`density/<unit>`** entry in `blueprint.remaining` per doc over budget,
     ...
     The next `/skill:vwf-blueprint` run clears the queue: the surveyor reports each
...
- **`18 → 19`** → **project role replaces project type**. Confined to
  ...
  4. **The IaC role is available but never inferred.** Do not retype any
     ...
     `/skill:vwf-architecture`. Write the token as **`iac`** — this migration ran under
...
- **`19 → 20`** → **the `iac` role rename, plus the own-repo rule**. Steps 1–3
  ...
  *proposal*, never an action.
  ...
  4. **An `iac` project must be its own repo** — independent, or a submodule of
     ...
     A decline is recorded and not re-proposed; `/skill:vwf-doctor` keeps reporting it
...
- **`20 → 21`** → **one mempalace config per product**. Every step is
  ...
  5. **Seed any protocol room the union is missing**, and rename a **near-miss**
     ...
     **every one path-derived and not one of them a protocol room** — mined
     content was searchable while every `/skill:vwf-handoff`, every findings
