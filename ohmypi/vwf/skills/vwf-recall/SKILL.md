...
Retrieve a handoff written by `/skill:vwf-handoff` and reconstruct enough context to
...
`next` is the reserved "resume where I left off" handoff `/skill:vwf-handoff` writes
...
- **Its continuation runs without a gate** — step 4 executes the Next prompt
  ...
  work. It is **left in place** afterwards; the following `/skill:vwf-handoff` (or
  `/skill:vwf-handoff next`) overwrites it.
...
Resolve `<project>` from the repo identity **exactly as `/skill:vwf-handoff` does**
...
`docs/memory/handoff/<name>.md` from disk (the `/skill:vwf-handoff` fallback) —
under the **main checkout** root, the parent directory of
`git rev-parse --git-common-dir`, which is where `/skill:vwf-handoff` writes it
...
do); if the repo's blueprint format is behind what vwf ships, nudge `/skill:vwf-setup`
...
- **Branch still exists, un-merged** → recreate the worktree for it via
  `/skill:vwf-git-workflow`, then continue there.
...
- **Yes** → proceed to execute that prompt (route through the matching `/vwf:`
  ...
  cap-paused `/skill:vwf-execute` run is the primary use of this command.
...
alone, so a re-run resumes the same point until the next `/skill:vwf-handoff`
overwrites it.
