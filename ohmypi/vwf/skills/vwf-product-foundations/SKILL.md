...
description: The foundational concerns every product must decide — users &
  ...
  /skill:vwf-architecture (the foundations checklist) and /skill:vwf-blueprint (expanding
...
product, never silently assumed: `/skill:vwf-architecture` walks this checklist as
...
`cross_cutting` token, and `/skill:vwf-blueprint` expands accepted ones into the
...
- **`/skill:vwf-architecture` (step 3c)** — walks the checklist. For each foundation:
...
- **`/skill:vwf-blueprint`** — for each accepted foundation, expand the contract into
...
- **`/skill:vwf-plan` / `execute`** — consume the contracts like any other conventions
