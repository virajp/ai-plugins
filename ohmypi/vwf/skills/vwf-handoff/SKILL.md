...
description: Capture the current session as a handoff document and file it to
  ...
  docs/memory/handoff/next.md, which /skill:vwf-recall resumes
...
the current context, and file it to **mempalace** so `/skill:vwf-recall` can retrieve
it later.
...
Named handoffs (`/skill:vwf-handoff auth-refactor`) keep their existing behavior
...
rule as `/skill:vwf-git-workflow`). **Do not push** — commit only, honoring the
never-push rule.
...
branch (`git branch --show-current`). `/skill:vwf-recall` reads these to re-enter the
...
`/skill:vwf-recall next` will report the same and wait. A `next` that auto-runs a
...
`# Handoff: <name>` header are how `/skill:vwf-recall` finds this drawer by `<name>`.
...
unreachable, and that `/skill:vwf-recall` will read the disk copy.
...
2. Write the same document verbatim to `docs/memory/handoff/next.md` under that
   ...
   `/skill:vwf-setup` maintains keeps it out of every diff.
...
`/skill:vwf-recall <name>` — `/skill:vwf-recall next` for the reserved one, noting it will
