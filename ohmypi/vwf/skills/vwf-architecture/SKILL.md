...
Invoke `/skill:vwf-git-workflow` to ensure an isolated local worktree before making
...
`stack` block**, because that block is what `/skill:vwf-doctor` checks the repo against
...
delegate to /skill:vwf-docs-sync scoped to this run's changes, so the
...
**If this command was invoked as a sub-step of `/skill:vwf-blueprint` or
`/skill:vwf-execute` (registry reconciliation):** return control to the parent run.
The parent pipeline commits via /skill:vwf-git-workflow; do not double-commit.

**Otherwise (standalone invocation):** commit via `/skill:vwf-git-workflow`.
