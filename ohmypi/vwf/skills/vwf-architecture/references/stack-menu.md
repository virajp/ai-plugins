...
the project block**, for every project: it is what `/skill:vwf-doctor` checks the repo
...
write one (`%%AI_PLUGINS_ROOT%%/assets/stack-adapter.md`, "Writing a stack
...
each of those writes a config `/skill:vwf-doctor` blocks on immediately, which is a
worse outcome than the halt because it arrives one command later. Why the menu is
closed at all is in `%%AI_PLUGINS_ROOT%%/assets/stack-vocabulary.md`.
