...
`/skill:vwf-setup` — the same definitions detection reads backward,
...
name a topology from `%%AI_PLUGINS_ROOT%%/assets/topologies/`; propose one with its
...
`/skill:vwf-doctor` reports the arrangement on every run, blocking
...
batching. Each round follows `%%AI_PLUGINS_ROOT%%/assets/elicitation.md`: name the
