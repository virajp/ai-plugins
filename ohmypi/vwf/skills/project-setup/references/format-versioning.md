...
submodule of the product parent — which `/skill:doctor` raises as a **blocking**
finding and `/skill:setup` offers to restructure. It ships with `config_format` 13,
...
- **`18 → 19`** → **project role replaces project type**. Confined to
  ...
  4. **The IaC role is available but never inferred.** Do not retype any
     ...
     `/skill:architecture`. Write the token as **`iac`** — this migration ran under
...
- **`19 → 20`** → **the `iac` role rename, plus the own-repo rule**. Steps 1–3
  ...
  *proposal*, never an action.
  ...
  4. **An `iac` project must be its own repo** — independent, or a submodule of
     ...
     A decline is recorded and not re-proposed; `/skill:doctor` keeps reporting it
