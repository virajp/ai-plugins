...
- **Don't duplicate tools.** Use `devtools:scaffold` for mise config and
  `/skill:readme` for the README — orchestrate, don't reimplement.
...
there is no `enforcement` entry for a stack. `/skill:architecture` owns this elicitation
...
runs; `/skill:doctor` keeps reporting it as blocking, which is the honest state.
...
**`/skill:readme`**; if it fails, report the error, offer to continue without it
...
- **per-project nuances** the run surfaced — a `coverage_target` override, a
  ...
  `docs/blueprint/registry.yaml`, written by `/skill:architecture` (format 19
...
- the **`stack` block for every project** and the repo-level **`repo.stack`**,
  ...
  **all three technology axes per project**: `template`, `backing_template` (a
  ...
  `/skill:doctor` with nothing to check;
