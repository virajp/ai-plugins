...
description: Two-way screen sync with the project's design tool. "prompt <flow>" writes one
  ...
  contract delta through /skill:vwf-blueprint — this skill never edits a flow doc
  itself.
---
...
**through `/skill:vwf-blueprint`, one confirmed delta at a time**. The blueprint stays
...
- No design system (either form) → "Screens reference the design system; run
  `/skill:vwf-design-system` first." Stop.
...
doc: `import` routes every accepted delta through `/skill:vwf-blueprint`.
