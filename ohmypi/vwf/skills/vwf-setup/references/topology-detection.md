...
finding, so the shared spine's `/skill:vwf-doctor` run halts on it: vwf's stack menu is
...
`/skill:vwf-architecture` proposes role and platforms from how each
...
`/skill:vwf-doctor` keeps it visible.
