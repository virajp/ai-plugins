...
`/skill:vwf-product` and `/skill:vwf-architecture`, which
...
1. **Tooling.** If the mise config is missing, invoke
   `/skill:scaffold`. If it fails, report the error and offer
...
`/skill:vwf-doctor` reads as early rather than as drift.
...
- **Harness detection** per `%%AI_PLUGINS_ROOT%%/assets/harness.md` — which
  ...
  `/skill:vwf-plan` injects their bootstrap when a cycle first needs
  one.
...
deliverable, and `/skill:vwf-doctor` is what keeps it visible.
