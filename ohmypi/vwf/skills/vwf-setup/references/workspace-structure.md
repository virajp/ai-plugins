...
`/skill:vwf-setup` presents them, exactly as it presents stack templates. The user
...
differs, and why closing the gap is worth the work. `/skill:vwf-setup`
moves no source file, in-repo or across a repo boundary, so the recommendation
is the whole deliverable; `/skill:vwf-doctor` is what keeps it visible
...
`/skill:vwf-architecture`. Adding a **repo** is a `members:` entry plus its
`.config/vwf-membership.yaml` (plus `git submodule add` under submodule
linkage), recorded by running `/skill:vwf-setup` from **inside** the new repo, where
...
`%%AI_PLUGINS_ROOT%%/assets/stack-adapter.md`; the shape of a language fact is in
[stack-vocabulary](%%AI_PLUGINS_ROOT%%/assets/stack-vocabulary.md). Restating them
...
project, presented by `/skill:vwf-architecture` and chosen by the
...
required to live in **its own repo** (`%%AI_PLUGINS_ROOT%%/assets/topologies/`) — the
