...
| Template      | `%%AI_PLUGINS_ROOT%%/assets/templates/design-system.md`                                  |
...
found. Run `/skill:vwf-architecture` first." If the registry has **no** UI-surface
...
project to the registry first** via `/skill:vwf-architecture` (then return here), or
(b) proceed anyway.
...
Per `%%AI_PLUGINS_ROOT%%/assets/memory.md`, recall prior design decisions and
...
- **No `design` on any UI project** → "No design tool configured. Set
  ...
  `config_format` 12 drift — nudge `/skill:vwf-setup` rather than reading it.
...
per `%%AI_PLUGINS_ROOT%%/assets/elicitation.md`. The import fills the doc;
...
`/skill:vwf-blueprint`. Never edit a flow doc from here.
...
project) routes through `/skill:vwf-architecture` — never a by-hand registry edit.
...
hand **all** git actions to `/skill:vwf-git-workflow` — the pin rides the same commit.
