...
(`docs/blueprint/design-system.md`, authored by `/skill:vwf-design-system`). Reference
...
- **State UX** — loading, empty (first-run vs no-results), error (placement +
  ...
  pass renders for visual review and `/skill:vwf-mockups` draws. Pin equally the
...
- **Components & rules** — the elements the screen displays (text, info, error
  ...
  by the row's code; `/skill:vwf-screens prompt` transcribes it into the design brief.
