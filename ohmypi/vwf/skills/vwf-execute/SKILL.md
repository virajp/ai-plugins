...
`/skill:vwf-plan` first." If `$ARGUMENTS` names no plan and more than one is active,
...
> `implementation: <state>`). Run `/skill:vwf-execute <file>` first."

No override flag — if reality differs from the stamp, heal it via `/skill:vwf-plan`
(its stamp-heal offer) or amend the blueprint via `/skill:vwf-blueprint`; never guess
...
(the run needs an artifact the old format lacks), **pause** for `/skill:vwf-setup` per
...
- **One plan, one worktree.** Via `/skill:vwf-git-workflow`, create a dedicated
...
- **All git via `/skill:vwf-git-workflow`.** Never run raw git. On **every** mid-run
...
The resume command is `/skill:vwf-recall next` after a **resource-cap** pause (which
ran a bare `/skill:vwf-handoff` first, writing the reserved `next` handoff), and
`/skill:vwf-execute <plan>` for every other pause (it resumes from the run journal per
the Resume check).
...
- **Resource caps** — context > 65%, 5-hour > 90%, or 7-day > 80% (a repo may
  **tighten** these — never loosen — via `.config/vwf.yaml`
  ...
  directive, run `/skill:vwf-handoff` with no argument to snapshot state as the
  reserved `next` handoff, then stop; resume later with `/skill:vwf-recall next`.
...
paused at a resource cap (`/skill:vwf-handoff` → `/skill:vwf-recall next`) picks up where it
left off.
...
1. **Preflight (interactive — the user is still present at invocation).** Run
   `/skill:vwf-doctor` scoped to the plan's projects. It reads each project's
...
2. **Worktree.** Invoke `/skill:vwf-git-workflow` to create the dedicated worktree,
   ...
   **In a `multi-repo` product, which repo the worktree is of follows the
   linkage** — `/skill:vwf-git-workflow` resolves it, and the two
...
6. **commit** — commit the step's work via `/skill:vwf-git-workflow`, **per the
...
1. **Architecture, environment, harness, docs & implementation stamps.**
   ...
   repo's human docs (README/CLAUDE.md) via /skill:vwf-docs-sync
...
- **Approve** → hand off to `/skill:vwf-git-workflow` for the merge/push sequence
...
doc**: blueprint holes → `/skill:vwf-blueprint` (the sweep re-stamps coverage); plan
holes → `/skill:vwf-plan` to re-derive the slice against the now-updated blueprint.
When a gap is reconciled, note its resolution back into the `gaps` room so a
later cycle's recall sees it as closed.

## Archive

After a merge with no open gaps, **tell the user to run `/skill:vwf-archive`** to
retire the completed plan. `/skill:vwf-archive` is user-only — you cannot invoke it,
...
`/skill:vwf-execute <next-plan>` — chained plans land one focused run at a time.
