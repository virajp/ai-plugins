...
## Canvas harvest (`/skill:vwf-feedback canvas`)
...
the user said there while designing screens (`/skill:vwf-screens`) or
...
1. **Resolve the scope.** Every registry project declaring a **screen platform**
   ...
   come from `/skill:vwf-screens` or `/skill:vwf-design-system`)." Stop.
...
3. **Delegate, one call per project:**
   `/skill:design-tools-import-conversations <project>`. One call per
...
graph-first per `%%AI_PLUGINS_ROOT%%/assets/graphify.md` (`graphify query` the
symptom to find the owning flow/entity/screens) instead of skimming blind.
**Recall** rooms `gaps` and `problems` per
`%%AI_PLUGINS_ROOT%%/assets/memory.md` — if this item is already known, say so
...
- **Behavior bug** → the blueprint is right, the code is wrong: file to room
  `gaps` (tagged by flow/entity) and offer `/skill:vwf-plan <slice>` for a fix cycle.
...
- **Metric reading** → append a dated row to the **Metric readings** appendix of
  ...
  reviewed contract). A **miss against target** → offer `/skill:vwf-product` to
...
- **UX issue** → record it against the screen's **home flow** — the `## Screens`
  ...
  `/skill:vwf-design-system` (language-level) or `/skill:vwf-blueprint <flow>`
  (screen-level).
- **Feature idea** → never straight to code: offer `/skill:vwf-product` (does it serve
...
Per `%%AI_PLUGINS_ROOT%%/assets/memory.md`: bugs/holes to room `gaps`,
...
Commit any doc edits via `/skill:vwf-git-workflow` (`docs:` or `blueprint(...)`
message). If the user accepted a fixing command, hand off to it now.

## Metric readings appendix (product.md)

Maintained by this command (and read by `/skill:vwf-product` on re-runs):

```markdown
## Metric readings

<!-- Dated log, appended by /skill:vwf-feedback — not part of the reviewed contract. -->
