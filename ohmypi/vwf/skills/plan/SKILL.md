...
description: Produce reviewable cycle plans as diffs for one slice of the
  ...
  /skill:doctor blocking finding across the chain's projects.
---
...
**Stack gate.** Once the chain is approved, run `/skill:doctor` scoped to every
...
- **Blocking only.** Unlike `/skill:execute` this gate does **not** ask about a
