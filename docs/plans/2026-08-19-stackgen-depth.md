...
1. **Codify the bar.** `plugins/stackgen/assets/kinds.md`: replace the
   language-bundle structure sketch with the 12-topic bar, the framework ruling
   (sources, materiality, composition), and the depth sizing
   (archetype-referenced). State that `database` and `cloud-provider` bars are
   **pending elicitation** (step 4) so the reviewer does not enforce a bar that
   was never settled.
2. **Generator: per-topic loop, Context7-first.** The generator reference
...
3. **Reviewer check 8 — coverage** in `stackgen-skill-reviewer`, per the ruling
   above.
4. **Shape the remaining bars with the user** (one at a time, MCQ where options
   ...
   settled.
5. **Regenerate the Rust proof at full depth** against the settled bar (~10+
   Context7 passes, one artifact per applicable topic, reviewer with check 8
   active, consent gate, landing). This is the acceptance test: the output
   should read like the `typescript` plugin's references, and a shallow topic
   should be caught by check 8, not by the user.
6. **Docs ship with the change**: `docs/plugins/stackgen.md` (kinds/depth
   section), CLAUDE.md rows if touched, and the stackgen plan doc's decision
   table gains a pointer to this plan.
...
- **Stack-vocabulary coverage audit (requested 2026-08-19).** Once stackgen is
  ...
  surface with no orphaned concepts from the plugin era.
