...
`/setup` presents them, exactly as it presents stack templates. The user
...
differs, and why closing the gap is worth the work. `/setup`
moves no source file, in-repo or across a repo boundary, so the recommendation
is the whole deliverable; `/doctor` is what keeps it visible
...
`/architecture`. Adding a **repo** is a `members:` entry plus its
`.config/vwf-membership.yaml` (plus `git submodule add` under submodule
linkage), recorded by running `/setup` from **inside** the new repo, where
...
project, presented by `/architecture` and chosen by the
