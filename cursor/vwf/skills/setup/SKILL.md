...
- **Don't duplicate tools.** `/scaffold` for mise config. **Never
  write a README by hand** — `/readme` owns it, and setup only names
  it in the chain.
...
`/doctor`'s job, and saying so is the whole report.
...
3. **Run /doctor** over the repo — it checks the config just written
...
4. **Approval gate & commit.** Summarize everything created and updated, plus
   ...
   `/git-workflow` with a `chore(vwf):` or `docs:` message.
...
6. **Chain forward.** Print the ordered chain and stop:
   ...
   `/design-system` (once a project declares a screen
   platform) → `/blueprint`, with
   `/readme` optional at any point. Offer to start
   `/product` now. **setup runs none of them** — each
...
the finding: `/doctor` keeps reporting it as a warning,
