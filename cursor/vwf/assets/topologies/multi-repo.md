...
product-wide mise config and the local stack. `/setup` runs
...
it could not inspect**. `/execute` is the exception and
...
`/doctor` raises a violation as **blocking**, and
`/setup` writes up the extraction as a recommendation — one
...
`/setup` from **inside the new repo**: finding no config
...
registry projects are `/architecture`'s to record.
