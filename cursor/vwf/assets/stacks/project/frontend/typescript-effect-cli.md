...
# Frontend (cli) — TypeScript · Effect CLI
...
`frontend` is the role because a CLI runs on the user's machine, like an app —
not because it has a GUI. Internal dev scripts are not this: a repo's own task
runner is tooling, and gets no project entry.
