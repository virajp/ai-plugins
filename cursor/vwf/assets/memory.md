...
`/setup` writes them; `/doctor` reports
...
exists to prevent, and `/doctor` blocks on it.
...
letting one keyword claim both. This is also the shape `/setup` meets when
...
**Neither `/setup` nor `/doctor` scans for actual secrets.** They check
