...
/git-workflow already relies on exactly this. **Under
...
because you run /plan and /execute from
...
**`host:` and `path:` must agree.** /doctor reads both
...
- **Docs only** — /product,
  ...
  /blueprint, /screens,
  /mockups, /feedback,
  /archive. These read the blueprint, which is wholly in the
  base repo. **Absence is not a condition for them** — never detect, never ask.
- **Code** — /plan, /execute,
  /doctor, /verify. These follow the
  sequence below.
...
- **This does not remove /execute's cross-repo write.**
...
- **graphify** — a graph per checkout, refreshed by that checkout's own hook.
  /doctor gates every **locally-present** repo; an absent
