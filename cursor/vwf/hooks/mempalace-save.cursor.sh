...
answer=$(printf '%s' "$payload" \
  ...
  | "$(dirname "$0")/mempalace-checkpoint.sh")

if printf '%s' "$answer" | jq -e '.hookSpecificOutput.updatedInput' >/dev/null; then
  jq -n --arg m 'Save a MemPalace checkpoint — write a session diary entry and persist it via the mempalace MCP tools, then continue.' \
