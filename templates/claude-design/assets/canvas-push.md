...
Shared by every vwf surface that talks to Claude Design: `<%= it.cmd("vwf:design-system") %>`
(token sheets, publish, import) and `<%= it.cmd("vwf:screens") %>` (surface resolution and the
...
never travel through here** — `<%= it.cmd("vwf:mockups") %>` and blueprint §6a render only into
...
CLAUDE.md (device frame, layout — written by `<%= it.cmd("vwf:screens") %>`); **two platforms
...
project — `<%= it.cmd("vwf:design-system") %>` imports *from* it; nothing in vwf pushes to it.)
...
1. Read `design.projects.<registry-project>.<platform>` from `.config/vwf.yaml`.
   ...
   primary-platform pin for every UI project — honor them and nudge `<%= it.cmd("vwf:setup") %>`
...
3. **Offer to pin** the resolved id under
   ...
   via `<%= it.cmd("vwf:git-workflow") %>` (`chore(vwf): pin/stamp design project`), riding the
