...
description: Return the Cloudflare stack templates this plugin offers, as a vwf
  menu payload. Invoked by <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> when `cloudflare` is
...
- **The project axis is not ours.** Cloudflare fronts and hosts code; it does
  ...
  menu entry never carries `platforms:` — that key is project-axis only.
