...
description: Return one Cloudflare stack template as a vwf template payload —
  ...
  <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from the cloudflare menu —
...
`<%= it.cmd("cloudflare:cloudflare-stack-menu") %>` lists, and nothing else.
