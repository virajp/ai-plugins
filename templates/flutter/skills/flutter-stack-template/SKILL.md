...
description: Return one Flutter stack template as a vwf template payload — its
  ...
  <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from the flutter menu —
...
`<%= it.cmd("flutter:flutter-stack-menu") %>` lists, and nothing else. An unknown slug is an
...
1. Read `<%= it.root %>/stacks/project/<slug>.md` — flat since format 22 — the template file,
