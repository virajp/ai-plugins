...
description: Return the datastore templates this plugin offers, as a vwf menu
  payload. Invoked by <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> when `datastore` is listed
...
- **This list is exhaustive.** If a provider is not here, the `datastore` plugin
  ...
  user describes. Do not fill the gap from general database knowledge.
