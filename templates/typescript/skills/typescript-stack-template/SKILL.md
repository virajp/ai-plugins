...
description: Return one TypeScript stack template as a vwf template payload —
  ...
  <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from the typescript menu —
...
`<%= it.cmd("typescript:typescript-stack-menu") %>` lists, and nothing else. An unknown slug is
...
1. Read the template file. Its path depends on the axis:
   - `project` → `<%= it.root %>/stacks/project/<slug>.md` (flat since format 22)
   - `deploy` / `repo` → `<%= it.root %>/stacks/<axis>/<slug>.md`
