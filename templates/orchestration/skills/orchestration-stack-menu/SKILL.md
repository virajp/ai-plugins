...
description: Return the async-orchestration templates this plugin offers, as a
  vwf menu payload. Invoked by <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> when
...
happens later (`<%= it.root %>/assets/contract.md`) and ships the self-hosted
...
in the product's own datastore (`<%= it.root %>/assets/contract.md`). That is not
