...
description: Return one Google Cloud stack template as a vwf template payload
  ...
  <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from the gcp menu — not a
...
contract. Valid slugs are the ones `<%= it.cmd("gcp:gcp-stack-menu") %>` lists: `firebase`,
...
1. Read `<%= it.root %>/stacks/<axis>/<slug>.md` — the template file.
