...
description: Read the design review conversation back from a project's design
  ...
  conversations payload. Invoked by <%= it.cmd("vwf:feedback") %> canvas as its design
...
blueprint doc — those are vwf's, and `<%= it.cmd("vwf:feedback") %>` does them.
...
returning anything: `<%= it.pluginRoot("vwf") %>/assets/design-adapter.md` (or
...
`design.tool` is drift for `<%= it.cmd("vwf:setup") %>`'s `12 → 13` migration to copy down;
...
| Tool            | Read                                                                                                    |
| --------------- | --------------------------------------------------------------------------------------------------------- |
| `claude-design` | [claude-design](<%= it.root %>/skills/design-tools-import-conversations/references/claude-design.md) |
| `lovable`       | [lovable](<%= it.root %>/skills/design-tools-import-conversations/references/lovable.md)             |
| `stitch`        | [stitch](<%= it.root %>/skills/design-tools-import-conversations/references/stitch.md)               |
...
- **Transcribe, do not interpret.** A remark is reported close to how it was
  ...
  `<%= it.cmd("vwf:feedback") %>`'s job, and doing it here would pre-empt a decision the
