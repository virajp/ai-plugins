...
description: Return one object-storage stack template as a vwf template payload
  ...
  that do. Invoked by <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from
...
Read `<%= it.root %>/assets/contract.md` and answer from it — the rule against
