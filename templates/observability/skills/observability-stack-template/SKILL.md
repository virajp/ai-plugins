...
description: Return one observability stack template as a vwf template payload —
  ...
  <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> after the user picks from the observability
...
`<%= it.cmd("observability:observability-stack-menu") %>` lists, and nothing
else.
