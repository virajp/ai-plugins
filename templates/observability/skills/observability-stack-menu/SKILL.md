...
description: Return the observability templates this plugin offers, as a vwf
  menu payload. Invoked by <%= it.cmd("vwf:architecture") %> and <%= it.cmd("vwf:setup") %> when `observability`
...
(`<%= it.root %>/assets/contract.md`) and ships the self-hosted sink that
...
- **This list is exhaustive.** If a backend is not here, the `observability`
  ...
  what the user describes. Do not fill the gap from general telemetry knowledge.
