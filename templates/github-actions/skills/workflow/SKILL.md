...
If there is **no mise config**, stop and tell the user to run `<%= it.cmd("mise:scaffold") %>`
...
`*-prod-v*` path, and the release *record* itself belongs to `<%= it.cmd("vwf:verify") %>`, not
CI.
