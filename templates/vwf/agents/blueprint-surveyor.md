...
description: Stateless coverage surveyor for the <%= it.cmd("vwf:blueprint") %> command. Invoked
  only by <%= it.cmd("vwf:blueprint") %> at the start of a sweep — do not delegate to it for
...
8. **Missing mandatory standard flow** — per
   `<%= it.root %>/assets/standard-flows.md`: for each project declaring a
