...
description: Writes or updates docs/blueprint/registry.yaml and
  docs/blueprint/architecture.md for the <%= it.cmd("vwf:architecture") %> command. Invoked only
  by <%= it.cmd("vwf:architecture") %> — do not delegate to it for general tasks. Writes the
...
- **Create mode** (no existing files): read
  `<%= it.root %>/assets/templates/registry.yaml` and
  `<%= it.root %>/assets/templates/architecture.md` as the starting
...
asset `<%= it.root %>/assets/capability-vocabulary.md`; read it and
