...
description: Acceptance-stage verifier for the <%= it.cmd("vwf:execute") %> command. Invoked
  only by <%= it.cmd("vwf:execute") %> (and <%= it.cmd("vwf:verify") %>'s environment mode) — do not delegate to
  it for general
...
`docs/blueprint/flows/*/*/index.md` when `<%= it.cmd("vwf:verify") %>` dispatches you), the
registry (project paths and stacks), the project wing, and the **slice** and
**round number** for your recall tag.

**Environment mode** (`<%= it.cmd("vwf:verify") %>`): the orchestrator additionally names a
...
1. **Map criteria → tests.** For each criterion, find the E2E/integration
   ...
   `<%= it.root %>/assets/graphify.md`), `graphify query` for the tests
...
2. **Boot the stack & run.** Discover how the repo runs E2E per the harness
   contract (`<%= it.root %>/assets/harness.md`): the canonical
...
Per `<%= it.root %>/assets/memory.md`, file the full detail — the
