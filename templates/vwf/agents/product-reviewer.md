...
description: Stateless completeness reviewer for the <%= it.cmd("vwf:product") %> command.
  Invoked only by <%= it.cmd("vwf:product") %> — do not delegate to it for general tasks.
...
`<%= it.cmd("vwf:feedback") %>` — it is exempt from the checklist (do not flag its rows as
