---
name: execute-coder
description: Code-stage implementer for the <%= it.cmd("vwf:execute") %> command. Invoked only
  by
  <%= it.cmd("vwf:execute") %> — do not delegate to it for general tasks. Implements the approved
...
`<%= it.root %>/assets/memory.md`), read the full findings, and address
...
`<%= it.root %>/assets/memory.md`, file the full gap to room `gaps` in
...
**Orient graph-first.** Per `<%= it.root %>/assets/graphify.md`, when a
...
2. **GREEN** — write the **minimum** code to make that test pass, walking the
   decision ladder in `<%= it.root %>/assets/minimalism.md` first: reuse
