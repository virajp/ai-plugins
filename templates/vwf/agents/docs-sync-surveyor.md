...
description: Stateless doc-drift surveyor for the <%= it.cmd("vwf:docs-sync") %> skill.
  Invoked only by <%= it.cmd("vwf:docs-sync") %> — do not delegate to it for general tasks.
...
2. **Graph first, files second.** Per
   `<%= it.root %>/assets/graphify.md`: when a graph exists,
