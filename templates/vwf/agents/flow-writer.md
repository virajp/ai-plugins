...
description: Writes or updates one flow folder (index.md contract + one
  ...
  <%= it.cmd("vwf:blueprint") %> command. Invoked only by <%= it.cmd("vwf:blueprint") %> — do not delegate to it
...
- `<%= it.root %>/assets/templates/flow.md` — the `index.md` contract;
- `<%= it.root %>/assets/templates/flow-platform.md` — each
...
- **Context** — the relevant `conventions.md` anchors and the registry block.
  ...
  `<%= it.root %>/assets/capability-vocabulary.md` ("the datastore").
...
3. **The catalog row** — update this flow's row in its project's section of
   ...
   `<%= it.root %>/assets/templates/flows-index.md` if it does not exist.
