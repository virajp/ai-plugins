...
description: Security reviewer for the <%= it.cmd("vwf:execute") %> command. Invoked only by
  <%= it.cmd("vwf:execute") %> — do not delegate to it for general tasks. Threat-models the
...
3. **Trace real call paths.** Per `<%= it.root %>/assets/graphify.md`,
