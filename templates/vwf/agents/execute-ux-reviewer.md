...
description: UX-conformance reviewer for the <%= it.cmd("vwf:execute") %> command. Invoked only
  by <%= it.cmd("vwf:execute") %>, and only for UI slices — do not
...
1. **Render — delegate, never improvise.** You do not know how to render
   ...
   contract (`<%= it.root %>/assets/stack-adapter.md`), passing the slice, the
...
Per `<%= it.root %>/assets/memory.md`, file the full findings —
