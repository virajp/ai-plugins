---
name: plan-surveyor
description: Stateless desired-vs-actual surveyor for the <%= it.cmd("vwf:plan") %> command.
  Invoked only by <%= it.cmd("vwf:plan") %> — do not delegate to it for general tasks. Reads one
...
2. **Graph first, files second.** Per
   `<%= it.root %>/assets/graphify.md`: if `graphify-out/graph.json`
