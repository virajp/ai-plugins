...
So `design.tool: lovable` resolves to `<%= it.cmd("lovable:lovable-import-screens") %>`. vwf
...
Before delegating, `<%= it.cmd("vwf:design-system") %>` and `<%= it.cmd("vwf:screens") %> import` **verify the
...
the flow platform template, so `<%= it.cmd("vwf:screens") %> import` can diff it directly
