...
| ---------- | ---------------------------------------------------------------------- | ----------------- |
| **Export** | `<%= it.cmd("vwf:screens") %> prompt` writes design briefs to `docs/prompts/screens/…` | **No**            |
...
| Skill                                                       | Returns                     |
| ------------------------------------------------------------- | --------------------------- |
| `<%= it.cmd("design-tools:design-tools-import-screens") %> <flow> <platform>` | a **screens payload**       |
| `<%= it.cmd("design-tools:design-tools-import-design-system") %>`             | a **design-system payload** |
| `<%= it.cmd("design-tools:design-tools-import-conversations") %> <project>`   | a **conversations payload** |
...
The third name is the newest and was added to close a real hole: `<%= it.cmd("vwf:feedback") %>
...
Before delegating, `<%= it.cmd("vwf:design-system") %>`, `<%= it.cmd("vwf:screens") %> import` and
`<%= it.cmd("vwf:feedback") %> canvas` **check the project's configured tool** rather than
...
Returned by `<%= it.cmd("design-tools:design-tools-import-screens") %> <flow> <platform>`. Shapes
match the flow platform template, so `<%= it.cmd("vwf:screens") %> import` can diff it
...
Returned by `<%= it.cmd("design-tools:design-tools-import-design-system") %>`. Shapes match the
design-system template's sections, so `<%= it.cmd("vwf:design-system") %>` can write the doc
from it.
...
Returned by `<%= it.cmd("design-tools:design-tools-import-conversations") %> <project>`, one call
per registry project. Feeds `<%= it.cmd("vwf:feedback") %> canvas`, which classifies and routes
...
- Routing every accepted delta through `<%= it.cmd("vwf:blueprint") %>` — an adapter never edits
  a flow doc.
