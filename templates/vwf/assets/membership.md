...
<%= it.cmd("vwf:git-workflow") %> already relies on exactly this. **Under
...
because you run <%= it.cmd("vwf:plan") %> and <%= it.cmd("vwf:execute") %> from
...
**`host:` and `path:` must agree.** <%= it.cmd("vwf:doctor") %> reads both
...
- **Docs only** — <%= it.cmd("vwf:product") %>,
  <%= it.cmd("vwf:architecture") %>, <%= it.cmd("vwf:design-system") %>,
  <%= it.cmd("vwf:blueprint") %>, <%= it.cmd("vwf:screens") %>,
  <%= it.cmd("vwf:mockups") %>, <%= it.cmd("vwf:feedback") %>,
  <%= it.cmd("vwf:archive") %>. These read the blueprint, which is wholly in the
  base repo. **Absence is not a condition for them** — never detect, never ask.
- **Code** — <%= it.cmd("vwf:plan") %>, <%= it.cmd("vwf:execute") %>,
  <%= it.cmd("vwf:doctor") %>, <%= it.cmd("vwf:verify") %>. These follow the
  sequence below.
...
- **This does not remove <%= it.cmd("vwf:execute") %>'s cross-repo write.**
...
- **mempalace** — under `linkage: siblings`, **one `mempalace.yaml` per repo**,
  ...
  get none. Full contract: `<%= it.root %>/assets/memory.md`.
- **graphify** — a graph per checkout, refreshed by that checkout's own hook.
  <%= it.cmd("vwf:doctor") %> gates every **locally-present** repo; an absent
  member is a blind spot, not a finding. Full contract:
  `<%= it.root %>/assets/graphify.md`.
