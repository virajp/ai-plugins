...
`.config/vwf-membership.yaml` instead (`<%= it.root %>/assets/membership.md`).
Written by `<%= it.cmd("vwf:setup") %>` and
...
`<%= it.cmd("vwf:doctor") %>` finding rather than a value recorded and then ignored. Since
...
projects: # per-project REALIZATION + nuances — no role/path keys, ever (those describe the system: registry.yaml)
  <project-name>:
    stack: # the CONCRETE technology, structured. Lives here (never registry.yaml) so the blueprint is structurally incapable of naming a vendor. Written for EVERY project, always — an absent block is drift, not "the default", because <%= it.cmd("vwf:doctor") %> cannot check what was never recorded
      template: project/<slug> # the PROJECT-axis template, from an INSTALLED stack plugin. NOT a default: <%= it.cmd("vwf:architecture") %> presents the menu and the user picks. `custom` was RETIRED in format 14 — the menu is the whole vocabulary, and a platform nothing fits halts rather than recording free text. Format 15 dropped the `<role>/` path segment: a template declares the platforms it serves in its own frontmatter (one template can serve several — a Flutter template covers mobile+tablet+desktop+webapp), which a directory name cannot express. The pin must COVER every platform this project declares in the registry
      backing_template: [
        <slug>,
...
| `linkage` / `members` | `setup` (elicited; each member's facts detected)                                                                                         | every command that resolves the base repo or a member — `plan`, `execute`, `doctor`, `verify`, `git-workflow` (`<%= it.root %>/assets/membership.md`) |
...
| `docs_sync`          | the user (hand-edited)                                                                                                                    | the <%= it.cmd("vwf:docs-sync") %> skill                                                                        |
...
- **A stamped config with no registry is a legal state**, not drift. `setup`
  ...
  from `<%= it.cmd("vwf:architecture") %>`. Between the two the product is
  simply **early**: `<%= it.cmd("vwf:doctor") %>` says so as information (*next
  `<%= it.cmd("vwf:product") %>`, then `<%= it.cmd("vwf:architecture") %>`*) and
...
- **`5 → 6` migration** (performed by `<%= it.cmd("vwf:setup") %>`): each
  ...
  kept.
- **`6 → 7` migration** (performed by `<%= it.cmd("vwf:setup") %>`): every flow
...
- **`8 → 9` migration** (performed by `<%= it.cmd("vwf:setup") %>`): every flow
...
- **`9 → 10` migration** (performed by `<%= it.cmd("vwf:setup") %>`): the **stack
...
- **`12 → 13` migration** (performed by `<%= it.cmd("vwf:setup") %>`): **every
...
- **`13 → 14` migration** (performed by `<%= it.cmd("vwf:setup") %>`): **the stack menu closes.**
  ...
  (`<%= it.root %>/assets/stack-vocabulary.md`). Two things this file
  ...
  1. **`template: custom` is retired**, on all four axes
     ...
     block `<%= it.cmd("vwf:doctor") %>` checks. When nothing on the menu fits, halt naming the
     two ways forward: install the stack plugin that has a fitting template, or
     write one (`<%= it.root %>/assets/stack-adapter.md`).
  ...
  **Bump `config_format` to `14` only once both hold.** A halted migration leaves
  ...
  drift **and** as a blocking `<%= it.cmd("vwf:doctor") %>` finding — the drift says the repo is
  behind, the blocking finding says vwf will not build against it meanwhile.

- **`14 → 15` migration** (performed by `<%= it.cmd("vwf:setup") %>`): **a
  ...
  3. **`.config/vwf-membership.yaml` is written into every member**, naming the
     ...
     (`<%= it.root %>/assets/membership.md`). Under submodule linkage this is
