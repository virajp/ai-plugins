...
declares, and the facts `<%= it.cmd("vwf:doctor") %>` checks are supplied by the
...
`<%= it.cmd("vwf:setup") %>` and `<%= it.cmd("vwf:execute") %>` both halt on it.
...
`<%= it.cmd("vwf:architecture") %>` can match a project's declared capabilities against them:

```yaml
---
axis: backing
name: <display name>
capabilities: [] # from <%= it.root %>/assets/capability-vocabulary.md
...
`<%= it.cmd("vwf:architecture") %>` presents every template for a project's `role` and the user
...
that has one, or write it (`<%= it.root %>/assets/stack-adapter.md`,
...
`harness` block for `<%= it.cmd("vwf:doctor") %>` to check — and the pipeline ran on with those
