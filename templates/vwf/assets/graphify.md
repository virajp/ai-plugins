...
>
> - **The CLI** is a hard requirement. Missing → `<%= it.cmd("vwf:doctor") %>` §8 reports it as
>   **blocking**, and `<%= it.cmd("vwf:setup") %>` and `<%= it.cmd("vwf:execute") %>` halt on it the way
>   `execute` already halts on a missing LSP.
> - **A graph is per-checkout**, and its absence *at a checkout root* is equally
>   blocking — `<%= it.cmd("vwf:setup") %>` is what resolves it, behind consent.
...
>   (`<%= it.root %>/assets/membership.md`), and gating on a repo the user
...
> `<%= it.cmd("vwf:setup") %>` builds graphs, behind explicit consent.
...
- `docs/memory/` — recall belongs to the memory layer
  (`<%= it.root %>/assets/memory.md`); a graph copy freezes at the last commit
...
`<%= it.cmd("vwf:setup") %>` writes the file: the standard set plus whatever
...
`<%= it.cmd("vwf:doctor") %>` §8 reports a missing one as a **degradation**,
