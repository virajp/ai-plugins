...
vocabulary**, and the **screen-naming rule**. Shared by `<%= it.cmd("vwf:blueprint") %>`
(elicitation and the sweep's coverage gate via the `blueprint-surveyor`),
`<%= it.cmd("vwf:screens") %>` and `<%= it.cmd("vwf:mockups") %>` (platform resolution), `<%= it.cmd("vwf:architecture") %>`
...
- **conditional (auth)** — `signin` is required when the project carries an
  **Auth & identity** capability in the registry (`third-party-auth`,
  ...
  the registry is missing the capability; reconcile via `<%= it.cmd("vwf:architecture") %>`,
...
(`<%= it.root %>/assets/templates/registry.yaml`). Of them, exactly **six are
...
`<%= it.cmd("vwf:screens") %>`, `<%= it.cmd("vwf:mockups") %>`, the canvas, or the scratchpad. A flow of a
