...
2. **CLAUDE.md** — the repo's agent instructions: stack claims, task names,
   layout, the vwf section (`<%= it.cmd("vwf:setup") %>` owns its shape; this sync only fixes
...
- **Find contradicted docs graph-first**: when the repo carries a knowledge
  graph, per `<%= it.root %>/assets/graphify.md` query it for the docs
...
- **Broad drift** (the README no longer resembles the repo): regenerate via
  `<%= it.cmd("vwf:readme") %>` instead of patching sentence by sentence.
- **Same commit flow**: the sync lands through `<%= it.cmd("vwf:git-workflow") %>` with the
