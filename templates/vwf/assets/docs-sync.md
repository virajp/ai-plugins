...
reality therefore ends by delegating to <%= it.cmd("vwf:docs-sync") %>, in the
...
<%= it.cmd("vwf:docs-sync") %> owns it: scope resolution, the
`docs-sync-surveyor` scan (graph-first), surgical edits, the
<%= it.cmd("vwf:readme") %> route for broad README drift, the change-log hook,
...
`docs:` via <%= it.cmd("vwf:git-workflow") %>.
