# Execute Stages (used by <%= it.cmd("vwf:execute") %>)

The stage pipeline, per-stage subagent contracts, and shared stage rules used by
`<%= it.cmd("vwf:execute") %>`. The invoking command owns the orchestration policy — when to
...
- **acceptance** — dispatch `execute-acceptance-verifier` (pass the plan's
  ...
  (`<%= it.root %>/assets/harness.md`) — the **orchestrator decides**
...
- **Loop on findings** — review/security issues loop back to `code` with the
  **tag**, re-commit via `<%= it.cmd("vwf:git-workflow") %>`, then re-review. Send **both**
...
- **The blueprint is the source of truth — code follows.** When landed code
  ...
  consciously amending the contract via `<%= it.cmd("vwf:blueprint") %>`.
...
1. **Architecture.** If the implementation introduced a topology change (new
   ...
   `<%= it.cmd("vwf:architecture") %>` for non-trivial changes. Edit the registry precisely; do
