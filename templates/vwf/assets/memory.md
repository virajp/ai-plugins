...
`<%= it.cmd("vwf:setup") %>` writes the `.gitignore` entries (`docs/memory/handoff/`,
...
> **`<%= it.cmd("vwf:handoff") %>` and `<%= it.cmd("vwf:recall") %>` never skip.** The handoff *is* the
...
- **room** — a **closed set of seven**. `decisions` (design/architecture
  ...
  progress, for resuming a paused run; see below), `doctor` (`<%= it.cmd("vwf:doctor") %>`'s
  ...
  `<%= it.cmd("vwf:handoff") %>` and `<%= it.cmd("vwf:recall") %>` own).
  ...
  **Never invent an eighth.** mempalace creates a room implicitly on first
  ...
  silence is why the set is closed and why `<%= it.cmd("vwf:doctor") %>` checks it.

## Repo config — `mempalace.yaml`

**One product, one wing — and one config file per mined tree, at its root.**
`<%= it.cmd("vwf:setup") %>` writes them; `<%= it.cmd("vwf:doctor") %>` reports
...
exists to prevent, and `<%= it.cmd("vwf:doctor") %>` blocks on it.
...
letting one keyword claim both. This is also the shape `<%= it.cmd("vwf:setup") %>` meets when
...
**Neither `<%= it.cmd("vwf:setup") %>` nor `<%= it.cmd("vwf:doctor") %>` scans for actual secrets.** They check
...
`<%= it.cmd("vwf:execute") %>` keeps a single drawer per plan in room `runs` (drawer =
...
resource cap (`<%= it.cmd("vwf:handoff") %>` → later `<%= it.cmd("vwf:recall") %> next`), a resumed run reads
