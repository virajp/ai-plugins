...
> `docs/blueprint/product.md`, authored by `<%= it.cmd("vwf:product") %>` — the Phase −1
...
<!-- The ordered "build next" list — flows first (the primary blueprint unit),
     ...
     <%= it.cmd("vwf:blueprint") %> and <%= it.cmd("vwf:plan") %> pick up first, and why. Re-rank on each <%= it.cmd("vwf:product") %> re-run;
