<!-- Merged into a project's CLAUDE.md by <%= it.cmd("vwf:setup") %>, preserving existing content.
...
**Order:** `<%= it.cmd("vwf:setup") %>` → `<%= it.cmd("vwf:product") %>` → `<%= it.cmd("vwf:architecture") %>` →
`<%= it.cmd("vwf:design-system") %>` (once a UI exists) → `<%= it.cmd("vwf:blueprint") %>` (a full-product sweep
— `plan` halts until its coverage stamp reads complete) → `<%= it.cmd("vwf:plan") %> <slice>` →
`<%= it.cmd("vwf:execute") %>` → `<%= it.cmd("vwf:archive") %>` — then, after you deploy, `<%= it.cmd("vwf:verify") %> <env>`
and `<%= it.cmd("vwf:feedback") %>` route what production says back into product/blueprint/plan.
...
`<%= it.cmd("vwf:mockups") %> [flow]` batch re-renders (e.g. after a design-system change);
`<%= it.cmd("vwf:feedback") %> canvas` harvests the canvas review conversation back into the
...
Re-run `<%= it.cmd("vwf:setup") %>` after upgrading vwf to migrate the docs to the latest
format.
