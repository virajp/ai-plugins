...
`<%= it.cmd("vwf:product") %>`, `<%= it.cmd("vwf:architecture") %>`, `<%= it.cmd("vwf:design-system") %>`, `<%= it.cmd("vwf:blueprint") %>`,
`<%= it.cmd("vwf:screens") %>`, `<%= it.cmd("vwf:plan") %>`, `<%= it.cmd("vwf:setup") %>`, and `<%= it.cmd("vwf:feedback") %>`. You own the user
...
`<%= it.root %>/assets/memory.md`: file the full point to mempalace room
...
transcript is what a later `<%= it.cmd("vwf:recall") %>` — or a reader of the mempalace
