...
— the capabilities behind the `acceptance` and `ux` stages and `<%= it.cmd("vwf:verify") %>`.
...
## Detection (used by `<%= it.cmd("vwf:setup") %>`)
...
- **New/empty repos** — `<%= it.cmd("vwf:setup") %>` scaffolds the harness as part of the chosen
  topology's layout (the selected stack templates describe it), in the same
  consent-gated migration plan.
- **Existing repos** — `<%= it.cmd("vwf:plan") %>` runs a **harness preflight**: read the stamp,
