...
(`<%= it.root %>/assets/design-adapter.md`), applied to the stack.
...
`stack` block (`<%= it.root %>/assets/vwf-config.md`):
...
So `stacks: [ gcp ]` resolves to `<%= it.cmd("gcp:gcp-stack-menu") %>`. vwf constructs every
...
Before delegating, `<%= it.cmd("vwf:architecture") %>`, `<%= it.cmd("vwf:setup") %>` and `<%= it.cmd("vwf:doctor") %>` **verify
...
the registry. `<%= it.cmd("vwf:architecture") %>` offers only covering
templates; `<%= it.cmd("vwf:doctor") %>` reports a pin that stopped covering as
...
via a browser driver". `<%= it.cmd("vwf:doctor") %>` checks the task exists; it never checks
*which* tool.
...
the template's, and reaching it means asking the plugin. `<%= it.cmd("vwf:plan") %>` sizes its
steps against that prose and `<%= it.cmd("vwf:execute") %>` writes code to it, so both resolve
it the same way:
