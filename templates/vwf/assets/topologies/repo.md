...
repo. This is
the one structural rule vwf enforces rather than offers: `<%= it.cmd("vwf:doctor") %>` raises a
violation as **blocking**, and `<%= it.cmd("vwf:setup") %>` offers a consent-gated restructure.
...
**monorepo** — one repo, several projects. Re-run `<%= it.cmd("vwf:setup") %>` to record the
