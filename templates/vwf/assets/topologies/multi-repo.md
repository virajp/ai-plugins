...
product-wide mise config and the local stack. `<%= it.cmd("vwf:setup") %>` runs
...
[membership](<%= it.root %>/assets/membership.md). There is no equivalent of a
...
it could not inspect**. `<%= it.cmd("vwf:execute") %>` is the exception and
halts — you cannot write code into a repo you do not have. The full sequence is
in [membership](<%= it.root %>/assets/membership.md).
...
`<%= it.cmd("vwf:doctor") %>` raises a violation as **blocking**, and
`<%= it.cmd("vwf:setup") %>` writes up the extraction as a recommendation — one
...
`<%= it.cmd("vwf:setup") %>` from **inside the new repo**: finding no config
there, Step 0 resolves `onboard`, and base-repo resolution
(`<%= it.root %>/assets/membership.md`) reaches the product from it. Its
registry projects are `<%= it.cmd("vwf:architecture") %>`'s to record.
