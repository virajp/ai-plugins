...
is a convention `<%= it.cmd("vwf:setup") %>` proposes, not a hard requirement — a repo that
...
rule vwf **enforces**: `<%= it.cmd("vwf:doctor") %>` raises a violation as a **blocking** finding,
and `<%= it.cmd("vwf:setup") %>` writes up the extraction as a recommendation. Everything else on
this page is a menu.

Declining it under `enforcement:` is honoured, and honoured honestly: the
finding drops to a warning `<%= it.cmd("vwf:doctor") %>` reports every run, so nothing halts and
