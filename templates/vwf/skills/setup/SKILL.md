...
`<%= it.cmd("vwf:blueprint") %>` sweep (offered, consent-gated) does the elicited fill. YAML
...
- **per-project nuances** the run surfaced — a `coverage_target` override, a
  ...
  lives only in `docs/blueprint/registry.yaml`, written by `<%= it.cmd("vwf:architecture") %>`
...
- the **`stack` block for every project** and the repo-level **`repo.stack`**,
  ...
  is what leaves `<%= it.cmd("vwf:doctor") %>` with nothing to check;
...
**Write the mempalace config.** Per `<%= it.root %>/assets/memory.md`,
