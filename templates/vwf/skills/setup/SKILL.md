...
You own the user conversation, per `<%= it.root %>/assets/elicitation.md` — one
...
- **Worktree-safe; all git via <%= it.cmd("vwf:git-workflow") %>.** Operate in an isolated
...
- **Don't duplicate tools.** `<%= it.cmd("devtools:scaffold") %>` for mise config. **Never
  write a README by hand** — `<%= it.cmd("vwf:readme") %>` owns it, and setup only names
  it in the chain.
...
against the shipped integers (`<%= it.root %>/assets/blueprint-format`, and the
current `config_format` named in `<%= it.root %>/assets/vwf-config.md`).
...
`<%= it.cmd("vwf:doctor") %>`'s job, and saying so is the whole report.
...
2. **Write `.config/vwf.yaml`.** `<%= it.root %>/assets/vwf-config.md` is
   ...
   `<%= it.root %>/assets/membership.md`; which members are on this machine is
   detected every run and never recorded.
3. **Run <%= it.cmd("vwf:doctor") %>** over the repo — it checks the config just written
   ...
   **Halt on a `blocking` finding, and revert the stamp** — delete
   ...
   (`<%= it.root %>/assets/stack-adapter.md`), and never invent a template to
   get past it.
   ...
   **Two exceptions.** A **declined graph build** is a settled choice, not an
   ...
   `<%= it.cmd("vwf:execute") %>` halts on it.
4. **Approval gate & commit.** Summarize everything created and updated, plus
   ...
   `<%= it.cmd("vwf:git-workflow") %>` with a `chore(vwf):` or `docs:` message.
5. **The graph offer.** Per `<%= it.root %>/assets/graphify.md`, `setup` is the
...
6. **Chain forward.** Print the ordered chain and stop:
   `<%= it.cmd("vwf:product") %>` → `<%= it.cmd("vwf:architecture") %>` →
   `<%= it.cmd("vwf:design-system") %>` (once a project declares a screen
   platform) → `<%= it.cmd("vwf:blueprint") %>`, with
   `<%= it.cmd("vwf:readme") %>` optional at any point. Offer to start
   `<%= it.cmd("vwf:product") %>` now. **setup runs none of them** — each
...
the finding: `<%= it.cmd("vwf:doctor") %>` keeps reporting it as a warning,
which is the honest state of a repo that chose its own shape.

**Persist the decisions.** Per `<%= it.root %>/assets/memory.md`, store the
