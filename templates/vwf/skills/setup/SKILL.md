...
`<%= it.root %>/assets/membership.md`. A `setup` run started inside a member
...
project's repo violates the own-repo rule (`<%= it.root %>/assets/topologies/`) — record
...
there is no `enforcement` entry for a stack. `<%= it.cmd("vwf:architecture") %>` owns this elicitation
...
- Run `<%= it.cmd("vwf:design-system") %>` only if some project declares a **screen platform**
...
(`<%= it.root %>/assets/stack-adapter.md`). Everything migrated up to
