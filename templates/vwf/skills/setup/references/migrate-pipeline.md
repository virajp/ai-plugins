...
| Source | Defines |
| --- | --- |
| `<%= it.root %>/assets/templates/` | the skeleton of every doc vwf writes, frontmatter included |
| `<%= it.root %>/assets/examples/blueprint/` | a worked, conformant bundle — the concrete "what good looks like" |
| the blueprint-authoring skill's bars | what a doc must *say* to be complete, and the density budgets |
| `<%= it.root %>/assets/vwf-config.md` | every `.config/vwf.yaml` key, and which are retired |
...
1. **Diff the tree against those four.** Every file that is missing, misplaced,
   ...
   vwf-standard excludes (`<%= it.root %>/assets/graphify.md`), is one entry
   like any other.
...
3. **Fan-outs are verified, never picked.** A retired token mapping to more than
   ...
   `<%= it.root %>/assets/elicitation.md` — one decision per round.
...
5. **Apply the approved plan**, then hand back to the shared spine — which
   ...
   `<%= it.cmd("vwf:doctor") %>`, in that order. The pipeline stamps nothing
