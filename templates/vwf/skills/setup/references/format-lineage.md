...
(`<%= it.root %>/assets/templates/`,
`<%= it.root %>/assets/examples/blueprint/`, the blueprint-authoring bars, and
`<%= it.root %>/assets/vwf-config.md`) and fix what differs. The table below
...
detectors** and nothing else. `<%= it.root %>/assets/format-check.md` compares
a repo's stamps against the shipped integers and nudges
`<%= it.cmd("vwf:setup") %>`. Nothing selects a migration path by them any more,
...
| a synonym flow slug (`login`, `dashboard`, …) | the standard slug in `<%= it.root %>/assets/standard-flows.md` | numbering | yes |
...
| a flat **list** at `projects.<name>.stack` | the structured block — `template`, `languages`, `frameworks`, `dependencies`, split per `<%= it.root %>/assets/stack-vocabulary.md` | config-key | |
...
(`<%= it.root %>/assets/stack-adapter.md`), so a retired path is repinned from a
...
3. **Confirm by MCQ**, one decision per round per
   `<%= it.root %>/assets/elicitation.md`, with the other candidates offered.
