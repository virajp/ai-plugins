...
[membership](<%= it.root %>/assets/membership.md) before anything else — a run
...
[membership](<%= it.root %>/assets/membership.md) is what handles the rest.
...
finding, so the shared spine's `<%= it.cmd("vwf:doctor") %>` run halts on it: vwf's stack menu is
closed to what the installed plugins declare, and it will not plan or build
against a language none of them covers
([stack-vocabulary](<%= it.root %>/assets/stack-vocabulary.md)). Detection
...
`<%= it.cmd("vwf:architecture") %>` proposes role and platforms from how each
...
`<%= it.cmd("vwf:doctor") %>` keeps it visible.
...
it declares (`<%= it.root %>/assets/stack-adapter.md`).
