...
`<%= it.cmd("vwf:product") %>` and `<%= it.cmd("vwf:architecture") %>`, which
...
1. **Tooling.** If the mise config is missing, invoke
   `<%= it.cmd("devtools:scaffold") %>`. If it fails, report the error and offer
...
4. **The graph ignore file.** Write `.graphifyignore` at the repo root with the
   vwf-standard excludes, per `<%= it.root %>/assets/graphify.md`. A blank repo
   has no repo-specific noise to detect, so the standard set is the whole file.
5. **CLAUDE.md.** Merge the vwf section from
   `<%= it.root %>/assets/templates/project-claude.md` into the repo's
...
`<%= it.cmd("vwf:doctor") %>` reads as early rather than as drift.
...
**Recall first.** Per `<%= it.root %>/assets/memory.md`, read room `decisions`
for prior topology, surface and stack confirmations — build on them rather than
re-asking. **Graph-first** where one exists: per
`<%= it.root %>/assets/graphify.md`, query `graphify-out/graph.json` for the
...
`<%= it.root %>/assets/membership.md`. A run started inside a member of an
...
1. **Manifest → candidate templates.** Each project directory's manifest
   ...
   for by contracted name per `<%= it.root %>/assets/stack-adapter.md`.
...
- **Docs scaffold** for whatever is missing: the blueprint skeletons from
  `<%= it.root %>/assets/templates/`, `flows/index.md`, `entities/index.md`, the
  empty `apis/` and `apis/released/` directories, `docs/plans/` and
  `docs/plans/archived/`.
- **The graph ignore file**, per `<%= it.root %>/assets/graphify.md`: the
  ...
  `multi-repo` product.
- **Harness detection** per `<%= it.root %>/assets/harness.md` — which
  ...
  `<%= it.cmd("vwf:plan") %>` injects their bootstrap when a cycle first needs
  one.
...
deliverable, and `<%= it.cmd("vwf:doctor") %>` is what keeps it visible.
