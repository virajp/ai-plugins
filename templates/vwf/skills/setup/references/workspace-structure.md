...
`<%= it.cmd("vwf:setup") %>` presents them, exactly as it presents stack templates. The user
...
| [`multi-repo`](<%= it.root %>/assets/topologies/multi-repo.md) | A group of repos, coordinated by a **base** repo              | the base repo           |
...
differs, and why closing the gap is worth the work. `<%= it.cmd("vwf:setup") %>`
moves no source file, in-repo or across a repo boundary, so the recommendation
is the whole deliverable; `<%= it.cmd("vwf:doctor") %>` is what keeps it visible
...
`<%= it.cmd("vwf:architecture") %>`. Adding a **repo** is a `members:` entry plus its
`.config/vwf-membership.yaml` (plus `git submodule add` under submodule
linkage), recorded by running `<%= it.cmd("vwf:setup") %>` from **inside** the new repo, where
...
`<%= it.root %>/assets/stack-adapter.md`; the shape of a language fact is in
[stack-vocabulary](<%= it.root %>/assets/stack-vocabulary.md). Restating them
...
project, presented by `<%= it.cmd("vwf:architecture") %>` and chosen by the
...
required to live in **its own repo** (`<%= it.root %>/assets/topologies/`) — the
