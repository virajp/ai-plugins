...
the user said to Claude Design while designing screens (`<%= it.cmd("vwf:screens") %>`) or
...
1. Gather every distinct pinned uuid from `.config/vwf.yaml`: the
   ...
   on the canvas to harvest (pins come from `<%= it.cmd("vwf:screens") %>` or
   `<%= it.cmd("vwf:design-system") %>`)." Stop.
