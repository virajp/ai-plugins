...
- Otherwise list the active plans from the base repo's `docs/plans/index.md`
  ...
  cloned (`<%= it.root %>/assets/membership.md`).
...
Report the moved paths. Commit the move via `<%= it.cmd("vwf:git-workflow") %>` (a
`docs(plan): archive <slice>` message); all git actions go through
<%= it.cmd("vwf:git-workflow") %>. Then, if mempalace is available, mark the plan's run journal
