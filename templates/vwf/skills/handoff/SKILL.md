...
description: Capture the current session as a handoff document and file it to
  ...
  handoff, to mempalace and to docs/memory/handoff/next.md, which <%= it.cmd("vwf:recall") %>
...
mempalace was unreachable, and that `<%= it.cmd("vwf:recall") %>` will read the disk copy.
