...
description: Capture the current session as a handoff document and file it to
  ...
  docs/memory/handoff/next.md, which <%= it.cmd("vwf:recall") %> resumes
  automatically. Use when the context window grows beyond ~60%.
argumentHint: "[<name> | next]"
model: sonnet
effort: medium
invocation: both
---
...
the current context, and file it to **mempalace** so `<%= it.cmd("vwf:recall") %>` can retrieve
it later.
...
| Template    | `<%= it.root %>/assets/templates/handoff.md`          |
...
Named handoffs (`<%= it.cmd("vwf:handoff") %> auth-refactor`) keep their existing behavior
...
rule as `<%= it.cmd("vwf:git-workflow") %>`). **Do not push** — commit only, honoring the
never-push rule.
...
`<%= it.root %>/assets/memory.md`.

### 4. Write the handoff document

Fill the template at `<%= it.root %>/assets/templates/handoff.md` from
...
branch (`git branch --show-current`). `<%= it.cmd("vwf:recall") %>` reads these to re-enter the
...
`<%= it.cmd("vwf:recall") %> next` will report the same and wait. A `next` that auto-runs a
...
`# Handoff: <name>` header are how `<%= it.cmd("vwf:recall") %>` finds this drawer by `<name>`.
...
unreachable, and that `<%= it.cmd("vwf:recall") %>` will read the disk copy.
...
1. Resolve the main checkout root — the parent directory of
   ...
   `<%= it.root %>/assets/memory.md`), so a copy written inside a worktree
...
2. Write the same document verbatim to `docs/memory/handoff/next.md` under that
   ...
   `<%= it.cmd("vwf:setup") %>` maintains keeps it out of every diff.
...
`<%= it.cmd("vwf:recall") %> <name>` — `<%= it.cmd("vwf:recall") %> next` for the reserved one, noting it will
