...
description: Maintain the always-current, full-product blueprint under
  ...
  <%= it.cmd("vwf:plan") %> halts without a complete stamp.
argumentHint: "[flow | entity]"
model: opus
effort: high
invocation: both
---
...
| Flow template    | `<%= it.root %>/assets/templates/flow.md`                                                                     |
| Platform templ.  | `<%= it.root %>/assets/templates/flow-platform.md`                                                            |
| Flow-cat. templ. | `<%= it.root %>/assets/templates/flows-index.md`                                                              |
| Entity template  | `<%= it.root %>/assets/templates/entity.md`                                                                   |
| Ent.-cat. templ. | `<%= it.root %>/assets/templates/entities-index.md`                                                           |
| Schema template  | `<%= it.root %>/assets/templates/schema.yaml`                                                                 |
| OpenAPI template | `<%= it.root %>/assets/templates/openapi.yaml`                                                                |
| Conv. template   | `<%= it.root %>/assets/templates/conventions.md`                                                              |
...
found. Run `<%= it.cmd("vwf:product") %>` first — the blueprint needs the goals every flow must
...
**Halt if it does not exist:** "No registry found. Run `<%= it.cmd("vwf:architecture") %>` first
...
`<%= it.root %>/assets/capability-vocabulary.md`).
...
`<%= it.root %>/assets/elicitation.md` (explore → scope-check → one
...
- **Engineering baseline (never re-elicited):** the defaults in
  `<%= it.root %>/assets/engineering-baseline.md` are settled — do not
...
- **Standard slugs, numbers & screen names:** when the journey being elicited
  matches an entry in `<%= it.root %>/assets/standard-flows.md` (splash,
...
- Minimalism rung 1 (`<%= it.root %>/assets/minimalism.md`, applied to
...
- **Conventions & environment (yours)** — update `docs/blueprint/conventions.md`
  ...
  `<%= it.root %>/assets/engineering-baseline.md` (`#baseline` — the 15
  default technical rules) and the **delivery pipeline** per
  `<%= it.root %>/assets/delivery-pipeline.md` (`#pipeline` — the
...
**registry** `docs/blueprint/registry.yaml` precisely — via `<%= it.cmd("vwf:architecture") %>`
...
moved, so the code is no longer known to match; the next `<%= it.cmd("vwf:plan") %>` for that
...
explicitly skipped, the drop stands and a later `<%= it.cmd("vwf:mockups") %> <flow>` re-renders
it. (Like the build stamp: a state-only edit, riding the same commit.)

**Persist.** Per `<%= it.root %>/assets/memory.md`, store this pass's
...
to `partial`. This stamp is what `<%= it.cmd("vwf:plan") %>` gates on.
...
After approval, hand **all** git actions to `<%= it.cmd("vwf:git-workflow") %>` — it owns
...
continue straight into `<%= it.cmd("vwf:plan") %>` for the highest-priority slice (from the
