...
Retrieve a handoff written by `<%= it.cmd("vwf:handoff") %>` and reconstruct enough context to
...
`next` is the reserved "resume where I left off" handoff `<%= it.cmd("vwf:handoff") %>` writes
...
- **Its continuation runs without a gate** — step 4 executes the Next prompt
  ...
  work. It is **left in place** afterwards; the following `<%= it.cmd("vwf:handoff") %>` (or
  `<%= it.cmd("vwf:handoff") %> next`) overwrites it.
...
Resolve `<project>` from the repo identity **exactly as `<%= it.cmd("vwf:handoff") %>` does**
...
`docs/memory/handoff/<name>.md` from disk (the `<%= it.cmd("vwf:handoff") %>` fallback) —
under the **main checkout** root, the parent directory of
`git rev-parse --git-common-dir`, which is where `<%= it.cmd("vwf:handoff") %>` writes it
...
`<%= it.root %>/assets/format-check.md` (as the other consuming commands
do); if the repo's blueprint format is behind what vwf ships, nudge `<%= it.cmd("vwf:setup") %>`
...
- **Branch still exists, un-merged** → recreate the worktree for it via
  `<%= it.cmd("vwf:git-workflow") %>`, then continue there.
...
- **Yes** → proceed to execute that prompt (route through the matching `/vwf:`
  ...
  cap-paused `<%= it.cmd("vwf:execute") %>` run is the primary use of this command.
...
alone, so a re-run resumes the same point until the next `<%= it.cmd("vwf:handoff") %>`
overwrites it.
