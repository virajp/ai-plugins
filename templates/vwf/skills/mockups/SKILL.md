...
argumentHint: "[flow, e.g. checkout — omit to sweep all screens]"
model: sonnet
effort: medium
invocation: user
---
...
**never a gate for `<%= it.cmd("vwf:plan") %>`**.
...
through `<%= it.cmd("vwf:blueprint") %> <flow>` or `<%= it.cmd("vwf:design-system") %>` — then re-run this
...
- No flow folders under `docs/blueprint/flows/` → "No blueprint found. Run
  `<%= it.cmd("vwf:blueprint") %>` first." Stop.
- No design system (neither file nor folder form) → "Screens reference the
  design system; run `<%= it.cmd("vwf:design-system") %>` first." Stop.
...
`<%= it.cmd("vwf:setup") %>` on drift (proceed unless the Screens/design-system artifacts this
command consumes are missing — then tell the user to run `<%= it.cmd("vwf:setup") %>` and stop).
...
to the repo's `.gitignore` and commit that one line via `<%= it.cmd("vwf:git-workflow") %>`
...
(`<%= it.root %>/assets/minimalism.md` — no speculative variant catalog).
...
Per `<%= it.root %>/assets/memory.md`, recall rooms `decisions` (design
...
`<%= it.cmd("vwf:blueprint") %> <flow>` or `<%= it.cmd("vwf:design-system") %>`, then re-render.
...
platforms. This is the render-currency state `<%= it.cmd("vwf:plan") %>`'s soft advisory reads
(and `<%= it.cmd("vwf:blueprint") %>` drops when a flow's Screens change unrendered).

**Persist.** Store the run outcome to mempalace room `decisions` per
`<%= it.root %>/assets/memory.md`. Skip silently if mempalace is
unavailable.
...
line from §1): hand that to `<%= it.cmd("vwf:git-workflow") %>` with a
