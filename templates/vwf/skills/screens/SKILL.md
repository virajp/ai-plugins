...
description: Two-way screen sync with Claude Design. "prompt <flow>" writes one
  ...
  contract delta through <%= it.cmd("vwf:blueprint") %> — this skill never edits a flow doc
  itself.
argumentHint: "[prompt <flow> | import [flow]]"
model: sonnet
effort: high
invocation: both
---
...
**through `<%= it.cmd("vwf:blueprint") %>`, one confirmed delta at a time**. The blueprint stays
...
| Prompt templ. | `<%= it.root %>/assets/templates/screen-prompt.md`                                                                                                                                                                                                                                     |
...
| Config        | `.config/vwf.yaml` — the `design:` block, per `<%= it.root %>/assets/vwf-config.md`                                                                                                                                                                                                    |

Adapter contract: `<%= it.root %>/assets/design-adapter.md` — the payload
...
- No design system (either form) → "Screens reference the design system; run
  `<%= it.cmd("vwf:design-system") %>` first." Stop.
...
**Format check.** Run `<%= it.root %>/assets/format-check.md`; nudge
`<%= it.cmd("vwf:setup") %>` on drift.
...
1. **Gather context.** Read the flow doc (steps, Screens rows — codes, states,
   ...
   format drift: say so and nudge `<%= it.cmd("vwf:setup") %>`, then elicit the platform set for
...
2. **Write one brief per platform** from the screen-prompt template to
   ...
   `<%= it.cmd("vwf:blueprint") %> <flow>` pass to pin them. **A brief is the full flow
...
6. **Stop.** The canvas session is the user's — iterate as long as needed; when
   satisfied, run `<%= it.cmd("vwf:screens") %> import <flow>`.
...
2. **List & match.** Resolve the surface and each in-scope UI project's
   ...
   **stitch page**. Match on the **full folder name**, never the number alone. A
   ...
   `<%= it.cmd("vwf:blueprint") %>`, or discard). A page matching neither → one MCQ per page
...
4. **Diff per flow.** Compare each **platform page** against the flow's contract
   ...
   **Scope every question** per §3a of
   `<%= it.root %>/assets/elicitation.md`. This surface needs it most: an
...
5. **Conventions fold.** Diff each canvas project's CLAUDE.md (step 3) against
   ...
   a blueprint doc — committed via `<%= it.cmd("vwf:git-workflow") %>`
   (`docs(prompts): fold canvas conventions`).
6. **Route — never edit here.**
   - **Accepted deltas** → hand each touched flow's verdict list to
     `<%= it.cmd("vwf:blueprint") %> <flow>` as that pass's input: the pass applies them under
   ...
   - **Proposed new flow** (confirmed in step 2 or 4) → scaffold a **draft**
     ...
     `<%= it.cmd("vwf:blueprint") %> <flow>` pass — pixels carry no steps or acceptance
