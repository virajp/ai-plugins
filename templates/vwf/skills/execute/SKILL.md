...
`<%= it.cmd("vwf:plan") %>` first." If `$ARGUMENTS` names no plan and more than one is active,
...
(`<%= it.root %>/assets/membership.md`). Read the index rather than walking the
...
> `implementation: <state>`). Run `<%= it.cmd("vwf:execute") %> <file>` first."

No override flag — if reality differs from the stamp, heal it via `<%= it.cmd("vwf:plan") %>`
(its stamp-heal offer) or amend the blueprint via `<%= it.cmd("vwf:blueprint") %>`; never guess
...
`<%= it.root %>/assets/format-check.md`. Since the run is autonomous: if
the format drift is **non-blocking**, log it and continue; if it is **blocking**
(the run needs an artifact the old format lacks), **pause** for `<%= it.cmd("vwf:setup") %>` per
...
`<%= it.root %>/assets/execute-stages.md` — follow them throughout. The
...
- **One plan, one worktree.** Via `<%= it.cmd("vwf:git-workflow") %>`, create a dedicated
...
- **All git via `<%= it.cmd("vwf:git-workflow") %>`.** Never run raw git. On **every** mid-run
...
- **Memory via mempalace (lean on it)** — follow
  `<%= it.root %>/assets/memory.md`. mempalace is the run's working
...
The resume command is `<%= it.cmd("vwf:recall") %> next` after a **resource-cap** pause (which
ran a bare `<%= it.cmd("vwf:handoff") %>` first, writing the reserved `next` handoff), and
`<%= it.cmd("vwf:execute") %> <plan>` for every other pause (it resumes from the run journal per
the Resume check).
...
- **Resource caps** — context > 65%, 5-hour > 90%, or 7-day > 80% (a repo may
  **tighten** these — never loosen — via `.config/vwf.yaml`
  ...
  directive, run `<%= it.cmd("vwf:handoff") %>` with no argument to snapshot state as the
  reserved `next` handoff, then stop; resume later with `<%= it.cmd("vwf:recall") %> next`.
...
Per `<%= it.root %>/assets/memory.md`, resolve the project **wing** and
...
paused at a resource cap (`<%= it.cmd("vwf:handoff") %>` → `<%= it.cmd("vwf:recall") %> next`) picks up where it
left off.
...
1. **Preflight (interactive — the user is still present at invocation).** Run
   `<%= it.cmd("vwf:doctor") %>` scoped to the plan's projects. It reads each project's
...
2. **Worktree.** Invoke `<%= it.cmd("vwf:git-workflow") %>` to create the dedicated worktree,
   ...
   **In a `multi-repo` product, which repo the worktree is of follows the
   linkage** — `<%= it.cmd("vwf:git-workflow") %>` resolves it, and the two
...
3. **Stack conventions.** Fetch the `conventions:` prose for every template this
   ...
   `<%= it.root %>/assets/stack-adapter.md` — deduped by slug, **once
...
6. **commit** — commit the step's work via `<%= it.cmd("vwf:git-workflow") %>`, **per the
...
1. **Architecture, environment, harness, docs & implementation stamps.**
   ...
   every other step.
2. **Persist.** Per `<%= it.root %>/assets/memory.md`, store the run's
...
- **Approve** → hand off to `<%= it.cmd("vwf:git-workflow") %>` for the merge/push sequence
...
doc**: blueprint holes → `<%= it.cmd("vwf:blueprint") %>` (the sweep re-stamps coverage); plan
holes → `<%= it.cmd("vwf:plan") %>` to re-derive the slice against the now-updated blueprint.
When a gap is reconciled, note its resolution back into the `gaps` room so a
later cycle's recall sees it as closed.

## Archive

After a merge with no open gaps, **tell the user to run `<%= it.cmd("vwf:archive") %>`** to
retire the completed plan. `<%= it.cmd("vwf:archive") %>` is user-only — you cannot invoke it,
...
`<%= it.cmd("vwf:execute") %> <next-plan>` — chained plans land one focused run at a time.
