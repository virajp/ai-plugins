...
edit, committed via `<%= it.cmd("vwf:git-workflow") %>`) and drop the element from the chain.
...
**Harness preflight.** Per `<%= it.root %>/assets/harness.md`, work out
...
visual render — recommend the user run `<%= it.cmd("vwf:mockups") %> <flow>` (a local scratchpad
render), or `<%= it.cmd("vwf:screens") %> import <flow>` when a
