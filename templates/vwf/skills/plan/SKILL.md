...
description: Produce reviewable cycle plans as diffs for one slice of the
  ...
  back through <%= it.cmd("vwf:blueprint") %> before writing — so no cycle builds on a gap.
  Requires the blueprint coverage stamp to read complete, and halts on any
  <%= it.cmd("vwf:doctor") %> blocking finding across the chain's projects.
argumentHint: "[flow/<name> | entity/<name> | <name>]"
model: opus
effort: high
invocation: both
---
...
**elicitation protocol** in `<%= it.root %>/assets/elicitation.md`.
...
| Plan template  | `<%= it.root %>/assets/templates/plan.md`                                    |
...
complete (`<remaining list, or 'never swept'>`). Run `<%= it.cmd("vwf:blueprint") %>` to finish
...
`<slice>`. Run `<%= it.cmd("vwf:blueprint") %>` first." A request that spans **several flows**
is not one slice — apply the scope check (§2 of
`<%= it.root %>/assets/elicitation.md`): decompose it, agree on order,
and run this pipeline per slice.

**Format check.** Run the preflight in
`<%= it.root %>/assets/format-check.md`; if the repo's blueprint format
is behind what vwf ships, nudge `<%= it.cmd("vwf:setup") %>` (proceed unless a needed artifact
is missing).

### 2. Resolve the dependency chain

**Recall first.** Per `<%= it.root %>/assets/memory.md`, recall prior
...
lives in, per `<%= it.root %>/assets/membership.md`. For every repo not on this
...
**Stack gate.** Once the chain is approved, run `<%= it.cmd("vwf:doctor") %>` scoped to every
...
- **Blocking only.** Unlike `<%= it.cmd("vwf:execute") %>` this gate does **not** ask about a
...
conventions* in `<%= it.root %>/assets/stack-adapter.md` — deduped by
...
`<%= it.root %>/assets/graphify.md` (falling back silently to direct
...
`<%= it.root %>/assets/minimalism.md` as you size each step: include a
...
pause, present the gap, and offer `<%= it.cmd("vwf:blueprint") %> <flow|entity>` (or
`<%= it.cmd("vwf:architecture") %>` for a registry hole). After that pass lands (and re-stamps
...
or the user consciously amends the contract via `<%= it.cmd("vwf:blueprint") %>` (which demotes
...
execution" section, and per `<%= it.root %>/assets/memory.md` recall room
`gaps` for the slice. When this plan is a reconcile loop-back from
`<%= it.cmd("vwf:execute") %>`, closing those plan holes is the point of the pass — fold each
...
per `<%= it.root %>/assets/membership.md`. A chain spanning two members already
...
`<%= it.cmd("vwf:archive") %>` and `<%= it.cmd("vwf:execute") %>` read to find a
...
- **Approve & execute** (the last element) — commit, then hand into
  `<%= it.cmd("vwf:execute") %>` **for the first unexecuted plan of the chain** (execute
...
- **Reject** — then either **Revise** (apply feedback, re-present, looping until
  ...
  state its exact path, offer via `<%= it.cmd("vwf:git-workflow") %>` to remove it and the
  worktree — and abandon the chain's downstream elements too, since they stand
  on it). Never let a plan doc silently linger.

**Persist.** Per `<%= it.root %>/assets/memory.md`, store the approved
...
After each approval, commit that plan via `<%= it.cmd("vwf:git-workflow") %>`. Use a
