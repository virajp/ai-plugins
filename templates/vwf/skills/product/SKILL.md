...
| Template | `<%= it.root %>/assets/templates/product.md` |
...
- **Exists → update mode.** Preserve confirmed content. Ask only about genuine
  ...
  `<%= it.cmd("vwf:feedback") %>` has been logging readings): a metric missing its target is a
...
`<%= it.root %>/assets/format-check.md`; on drift, **nudge** `<%= it.cmd("vwf:setup") %>`
...
Elicit following `<%= it.root %>/assets/elicitation.md` — one decision
...
<%= it.cmd("vwf:docs-sync") %> scoped to this run's changes, so the README's
...
Commit via `<%= it.cmd("vwf:git-workflow") %>` with a `blueprint(product):` message, e.g.:
