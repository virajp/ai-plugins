...
(`<%= it.root %>/assets/stack-adapter.md`). A table here would be a second copy
...
**One entry per platform is not a default.** `<%= it.cmd("vwf:architecture") %>` presents the
...
`<%= it.cmd("vwf:doctor") %>` checks and the old directory keying could not state.
