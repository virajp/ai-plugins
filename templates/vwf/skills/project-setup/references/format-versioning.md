...
in `<%= it.root %>/assets/blueprint-format`. The workflow commands
self-check the repo stamp against it via
`<%= it.root %>/assets/format-check.md` and nudge `<%= it.cmd("vwf:setup") %>` on drift —
...
finding and `<%= it.cmd("vwf:setup") %>` offers to restructure. It ships with `config_format` 14,
...
wording is a product decision. `<%= it.cmd("vwf:screens") %> prompt` transcribes the block into
...
and `<%= it.cmd("vwf:screens") %> import` match on. Format 10 = format 9 **plus** the
...
- **API contracts** move to `docs/blueprint/apis/<project>.openapi.yaml` — one
  ...
  the frozen production snapshots `<%= it.cmd("vwf:verify") %>` writes; from the first snapshot
...
`docs/blueprint/product.md` (type **`vwf-product`**, authored by `<%= it.cmd("vwf:product") %>`)
...
- **`4 → 5`** → author `docs/blueprint/product.md` via `<%= it.cmd("vwf:product") %>` (elicit —
  ...
  `<%= it.cmd("vwf:blueprint") %>`** (the reviewer enforces it on touch) — the migration does
...
- **`8 → 9`** → the process-based restructure, in **two phases**:
  ...
  **Phase 2 — elicited fill** (not this migration): the scaffold cannot invent
  ...
  flows that never existed — offer `<%= it.cmd("vwf:blueprint") %>` (consent-gated); coverage
...
- **`9 → 10`** → **project-grouped, execution-ordered flows**:
  ...
  5. Rewrite flow identifiers in `.config/vwf.yaml`: `design.flows_pushed`
     ...
     old names — the next `<%= it.cmd("vwf:mockups") %>` sweep or `<%= it.cmd("vwf:screens") %>` session renames
...
- **`10 → 11`** → **device-grouped flows + pinned screen codes**:
  ...
  3. In-car variants recorded as Screens-row deviations under the format-10
     ...
     pending in-car flows in `remaining:`, and offer `<%= it.cmd("vwf:blueprint") %>`
...
- **`11 → 12`** → **screen components as contract**: every Screens row gains a
  **Components block** (per the flow template) — the elements the screen
  ...
  by `<%= it.cmd("vwf:blueprint") %>`** (the reviewer enforces the block on touch) — a missing
...
- **`14 → 15`** → the **platform-file restructure + designated numbering**. The
  ...
  1. **Renumber to the bands** (per
     `<%= it.root %>/assets/standard-flows.md`). Per registry project,
  ...
  **not** mechanical and is deferred, on the `9` precedent (coverage stays
  ...
  5. **Queue the density pass — the sweep performs it.** Do not condense during
     ...
     **`density/<unit>`** entry in `blueprint.remaining` per doc over budget,
     ...
     The next `<%= it.cmd("vwf:blueprint") %>` run clears the queue: the surveyor reports each
...
- **`16 → 18`** → **project-scoped release tags** (17 is skipped, on the `13`
  ...
  1. **Re-seed `conventions.md#pipeline`** from
     `<%= it.root %>/assets/delivery-pipeline.md`, replacing the two tag
  ...
  2. **Report existing release workflows, do not rewrite them.** List any
     ...
     `<%= it.cmd("cicd:workflow") %>`'s job, not the migration's, and the old tags
...
repo: increment `<%= it.root %>/assets/blueprint-format`, add the `N→N+1`
