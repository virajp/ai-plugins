...
argumentHint: "[design-system id — omit to let the adapter resolve it]"
model: sonnet
effort: high
invocation: both
---
...
shape, per `<%= it.root %>/assets/design-adapter.md`. The canvas is where
...
| Template      | `<%= it.root %>/assets/templates/design-system.md`                                  |
...
found. Run `<%= it.cmd("vwf:architecture") %>` first." If the registry has **no** UI-surface
...
project to the registry first** via `<%= it.cmd("vwf:architecture") %>` (then return here), or
(b) proceed anyway.
...
Per `<%= it.root %>/assets/memory.md`, recall prior design decisions and
...
`<%= it.root %>/assets/design-adapter.md`, the tool is a **per-project**
...
- **No `design` on any UI project** → "No design tool configured. Set
  ...
  `config_format` 12 drift — nudge `<%= it.cmd("vwf:setup") %>` rather than reading it.
...
per `<%= it.root %>/assets/elicitation.md`. The import fills the doc;
...
`<%= it.cmd("vwf:blueprint") %>`. Never edit a flow doc from here.
...
project) routes through `<%= it.cmd("vwf:architecture") %>` — never a by-hand registry edit.
...
hand **all** git actions to `<%= it.cmd("vwf:git-workflow") %>` — the pin rides the same commit.
