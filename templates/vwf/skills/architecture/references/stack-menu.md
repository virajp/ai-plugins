...
write one (`<%= it.root %>/assets/stack-adapter.md`, "Writing a stack
...
each of those writes a config `<%= it.cmd("vwf:doctor") %>` blocks on immediately, which is a
worse outcome than the halt because it arrives one command later. Why the menu is
closed at all is in `<%= it.root %>/assets/stack-vocabulary.md`.
