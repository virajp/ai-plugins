...
`<%= it.root %>/assets/templates/registry.yaml`, and the screen-platform
semantics in `<%= it.root %>/assets/standard-flows.md`:
...
| Platform(s) | Obliges |
| --- | --- |
| `site` `webapp` `desktop` `mobile` `tablet` `auto` | **screen platforms** — design system mandatory, standard flows mandated, one `<platform>.md` per flow, `<%= it.cmd("vwf:screens") %>` briefs, canvas pins, mockups |
