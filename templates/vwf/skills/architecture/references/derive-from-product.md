...
`<%= it.cmd("vwf:setup") %>` — the same definitions detection reads backward,
...
name a topology from `<%= it.root %>/assets/topologies/`; propose one with its
...
`<%= it.cmd("vwf:doctor") %>` reports the arrangement on every run, blocking
...
batching. Each round follows `<%= it.root %>/assets/elicitation.md`: name the
