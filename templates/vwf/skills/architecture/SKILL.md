...
- **Absent but `architecture.md` exists with an embedded Project Registry** →
  the repo is pre-format-16. Nudge `<%= it.cmd("vwf:setup") %>` to run the `15 → 16` migration
...
halt.** Architecture is a prerequisite of `<%= it.cmd("vwf:setup") %>`'s own migration, so it
...
plainly and record what they chose: `<%= it.cmd("vwf:doctor") %>` will raise it as blocking and
`<%= it.cmd("vwf:setup") %>` will offer the restructure. Never restructure from here.
...
(`<%= it.root %>/assets/stack-vocabulary.md`), each its own menu:
...
the project block**, for every project: it is what `<%= it.cmd("vwf:doctor") %>` checks the repo
...
closed vocabulary in `<%= it.root %>/assets/stack-vocabulary.md` — offer
...
- Every registry project has a `projects.<name>.stack` block in
  ...
  (`<%= it.root %>/assets/stack-vocabulary.md`). A flat list, an absent
