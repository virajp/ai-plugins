...
argumentHint: "(no args; detects create vs update)"
model: sonnet
effort: high
invocation: both
---
...
| Reg. templ.  | `<%= it.root %>/assets/templates/registry.yaml`   |
| Arch templ.  | `<%= it.root %>/assets/templates/architecture.md` |
...
Invoke `<%= it.cmd("vwf:git-workflow") %>` to ensure an isolated local worktree before making
...
- **Absent but `architecture.md` exists with an embedded Project Registry** →
  the repo is pre-format-16. Nudge `<%= it.cmd("vwf:setup") %>` to reconcile the tree to the
...
- **Absent, with no `product.md` → recommend `<%= it.cmd("vwf:product") %>` first.** The
  outcome contract is required before `<%= it.cmd("vwf:blueprint") %>` either way, and writing it
...
is behind what vwf ships, **nudge** `<%= it.cmd("vwf:setup") %>` and **always proceed — never
halt.** `<%= it.cmd("vwf:setup") %>` prints the chain forward and runs none of it, so this
...
**Graph-first grounding.** Per `<%= it.root %>/assets/graphify.md`, when
...
`<%= it.root %>/assets/elicitation.md`: one decision per
...
First **read `<%= it.root %>/assets/capability-vocabulary.md`** — its
...
`<%= it.root %>/assets/topologies/`; it holds under all three topologies,
...
plainly and record what they chose: `<%= it.cmd("vwf:doctor") %>` raises it as blocking — a
decline recorded under `enforcement:` drops it to a warning reported every run —
and `<%= it.cmd("vwf:setup") %>` writes the extraction up as a recommendation, never a move.
...
`stack` block**, because that block is what `<%= it.cmd("vwf:doctor") %>` checks the repo against
...
- Every registry project has a `projects.<name>.stack` block in
  ...
  template one ships (`<%= it.root %>/assets/stack-vocabulary.md`). A token or pin
...
**Persist.** Once the checks pass, per `<%= it.root %>/assets/memory.md`
...
delegate to <%= it.cmd("vwf:docs-sync") %> scoped to this run's changes, so the
