...
- **Never install anything, and never build anything.** Report the command; the
  ...
  long LLM-driven job reserved for `<%= it.cmd("vwf:setup") %>`.
...
same treatment `<%= it.cmd("vwf:verify") %>` gives a criterion it already knows is failing. Skip
...
asset). Drift → report the delta and nudge `<%= it.cmd("vwf:setup") %>`; it is never doctor's
job to migrate.
...
`cicd`, without which `<%= it.cmd("cicd:workflow") %>` has to ask on every run. Report each as
drift naming the project and the axis, and nudge `<%= it.cmd("vwf:architecture") %>` — never
...
`<%= it.root %>/assets/topologies/`. The remedy is
`<%= it.cmd("vwf:setup") %>`, which offers the consent-gated restructure; doctor reports and
...
→ `10` drift; report and nudge `<%= it.cmd("vwf:setup") %>`.

### 6. Harness and health

Per `<%= it.root %>/assets/harness.md`, check every capability the
...
capability marked `false` is a **recorded gap, not a finding**: `<%= it.cmd("vwf:plan") %>`
...
starts a server or calls a deployed environment; that is `<%= it.cmd("vwf:verify") %>`'s job.
