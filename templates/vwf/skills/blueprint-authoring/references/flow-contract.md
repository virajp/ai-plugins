...
vocabulary (`<%= it.root %>/assets/standard-flows.md`) takes that exact
...
canvas frames are named by and `<%= it.cmd("vwf:screens") %> import` matches on; stable once
...
`<%= it.cmd("vwf:verify") %>` re-runs against deployed environments).
