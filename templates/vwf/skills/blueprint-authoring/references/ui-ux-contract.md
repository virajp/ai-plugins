...
(`docs/blueprint/design-system.md`, authored by `<%= it.cmd("vwf:design-system") %>`). Reference
...
- **State UX** — loading, empty (first-run vs no-results), error (placement +
  ...
  pass renders for visual review and `<%= it.cmd("vwf:mockups") %>` draws. Pin equally the
...
- **Components & rules** — the elements the screen displays (text, info, error
  ...
  by the row's code; `<%= it.cmd("vwf:screens") %> prompt` transcribes it into the design brief.
