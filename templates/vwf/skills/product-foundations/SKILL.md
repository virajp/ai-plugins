...
description: The foundational concerns every product must decide — users &
  ...
  <%= it.cmd("vwf:architecture") %> (the foundations checklist) and <%= it.cmd("vwf:blueprint") %> (expanding
...
product, never silently assumed: `<%= it.cmd("vwf:architecture") %>` walks this checklist as
...
`cross_cutting` token, and `<%= it.cmd("vwf:blueprint") %>` expands accepted ones into the
...
- **`<%= it.cmd("vwf:architecture") %>` (step 3c)** — walks the checklist. For each foundation:
...
- **`<%= it.cmd("vwf:blueprint") %>`** — for each accepted foundation, expand the contract into
...
- **`<%= it.cmd("vwf:plan") %>` / `execute`** — consume the contracts like any other conventions
