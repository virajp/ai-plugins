...
- **Don't duplicate tools.** Use `devtools:scaffold` for mise config and
  `/vwf:readme` for the README — orchestrate, don't reimplement.
...
`${CLAUDE_PLUGIN_ROOT}/assets/membership.md`. A `setup` run started inside a member
...
project's repo violates the own-repo rule (`${CLAUDE_PLUGIN_ROOT}/assets/topologies/`) — record
...
there is no `enforcement` entry for a stack. `/vwf:architecture` owns this elicitation
...
runs; `/vwf:doctor` keeps reporting it as blocking, which is the honest state.
The rule and its rationale live in `${CLAUDE_PLUGIN_ROOT}/assets/topologies/`.
...
- Run `/vwf:design-system` only if some project declares a **screen platform**
...
**`/vwf:readme`**; if it fails, report the error, offer to continue without it
...
- **per-project nuances** the run surfaced — a `coverage_target` override, a
  ...
  `docs/blueprint/registry.yaml`, written by `/vwf:architecture` (format 19
...
- the **`stack` block for every project** and the repo-level **`repo.stack`**,
  ...
  **all three technology axes per project**: `template`, `backing_template` (a
  ...
  `/vwf:doctor` with nothing to check;
...
(`${CLAUDE_PLUGIN_ROOT}/assets/stack-adapter.md`). Everything migrated up to
