...
So `design.tool: lovable` resolves to `/lovable:lovable-import-screens`. vwf
...
Before delegating, `/vwf:design-system` and `/vwf:screens import` **verify the
...
the flow platform template, so `/vwf:screens import` can diff it directly
