...
description: Acceptance-stage verifier for the /execute command. Invoked
  only by /execute (and /verify's environment mode) — do not delegate to
  it for general
...
`docs/blueprint/flows/*/*/index.md` when `/verify` dispatches you), the
registry (project paths and stacks), the project wing, and the **slice** and
**round number** for your recall tag.

**Environment mode** (`/verify`): the orchestrator additionally names a
...
1. **Map criteria → tests.** For each criterion, find the E2E/integration
   ...
   `%%AI_PLUGINS_ROOT%%/assets/graphify.md`), `graphify query` for the tests
...
Per `%%AI_PLUGINS_ROOT%%/assets/memory.md`, file the full detail — the
