...
6. Write `docs/architecture.md` from
   `templates/architecture.md`. Fill the human prose sections from step 4 and
   the ` ```yaml ` Project Registry block from step 5. Keep the prose and the
   registry in sync — every project in the registry must be described in the
