...
components:
- deploy-target/container-image@generated
...
tests plus its dependents'. CI itself belongs to the `cicd` plugin; no build
service of the host's is part of this stack, so there is exactly one place a
pipeline is defined.
