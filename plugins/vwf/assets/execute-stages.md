...
- **ux** — dispatch `execute-ux-reviewer` (pass the changed screens from the
  ...
  Screens section(s)
  (`docs/blueprint/flows/<project>/<device>/<NNN>-<flow>/index.md`), the UI
  project's registry entry, the wing, and the **slice** and **round number**).
  It renders the changed screens via the repo's own dev server + Playwright (per
  state where drivable), judges them against the design system and the Screens
