<purpose>
Ensure work happens in an isolated, local workspace. Prefer the platform's native
worktree tools; fall back to a manual git worktree only when no native tool is
available. Worktrees are always local to the repository and branch from the
current branch.
</purpose>

<user-story>
As an engineer (or an invoking skill), I want work isolated in a local worktree
before any changes are made, so that the current branch is protected from
in-progress work and a clean baseline is verified first.
</user-story>

<when-to-use>
- Starting feature work that needs isolation from the current workspace
- Before executing an implementation plan
- Invoked by another skill (doc-*, spec-plan, exec-plan) at the start of its flow
</when-to-use>
...
<step name="detect_existing_isolation" priority="first">
...
linked worktree. Skip directly to `project_setup`. Do NOT create another
worktree.
...
declines consent, work in place and skip to `project_setup`.
</step>

<step name="create_isolated_workspace">
...
flag. If you have one, use it and skip to `project_setup`.
...
Only proceed to 1b if you have **no native worktree tool** available.

### 1b. Git Worktree Fallback

**Only use this if 1a does not apply.**
...
<step name="verify_clean_baseline">
...
<output>
An isolated, local workspace (native worktree or `.worktrees/<branch>`) on a
branch created from the current branch, with dependencies installed and a
verified clean test baseline — or, where isolation was declined or blocked, work
proceeding in place with the same setup and baseline checks.
</output>

<acceptance-criteria>
- [ ] Existing isolation detected before any worktree was created
- [ ] Native worktree tool preferred; git fallback only when none is available
- [ ] Worktree is local (`.worktrees/`) and the directory is git-ignored
- [ ] New branch was created from the current branch, not the default branch
- [ ] `--no-verify` was not used without explicit user consent
- [ ] Dependencies installed and clean test baseline verified (or failures surfaced)
</acceptance-criteria>
...
| Situation                      | Action                                                    |
| ------------------------------ | --------------------------------------------------------- |
| Already in a linked worktree   | Skip creation (`detect_existing_isolation`)               |
...
- **Problem:** Using `git worktree add` when the platform already provides
  isolation.
- **Fix:** `detect_existing_isolation` detects existing isolation. 1a defers to
  native tools. This is the #1 mistake — if you have a native worktree tool, use
  it.

### Skipping isolation detection

- **Problem:** Creating a nested worktree inside an existing one.
- **Fix:** Always run `detect_existing_isolation` before creating anything.
...
- Create a worktree when `detect_existing_isolation` detects existing isolation.
- Use `git worktree add` when you have a native worktree tool — use the native
  tool.
- Skip 1a by jumping straight to 1b's git commands.
