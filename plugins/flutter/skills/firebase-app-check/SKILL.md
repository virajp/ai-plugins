...
description:
  Protect your Flutter app's Firebase backend resources with Firebase App Check
  — platform attestation providers, activation order, debug providers, token
  listeners, and Console enforcement. Use when enforcing that only genuine app
  builds can access Firestore, Storage, or other Firebase services, or when
  configuring debug providers for testing.
license: MIT
