...
`/typescript:typescript-stack-menu` lists, and nothing else. An unknown slug is
...
1. Read the template file. Its path depends on the axis:
   - `project` → `${CLAUDE_PLUGIN_ROOT}/stacks/project/<slug>.md` (flat since format 22)
   - `deploy` / `repo` → `${CLAUDE_PLUGIN_ROOT}/stacks/<axis>/<slug>.md`
