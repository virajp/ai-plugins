...
| `stackgen` | `plugins/stackgen` | The **principles-driven stack materializer** — implements the vwf stack-adapter contract with a per-component dispatch rule (a component a shipped pack covers is copied verbatim; an uncovered one is generated behind the `stackgen-skill-reviewer` gate), landing in the repo's committed `.claude/` tree plus three consent-gated targets outside it. The only stack plugin, and the home of the repo toolchain and gate doctrine since `devtools` dissolved into it. Its shape, the consent tiers, the pack rules and the inventory pointers are the `stackgen-plugin` skill, which auto-applies under `plugins/stackgen/` |
