...
- `assets/` — the shared doctrine and data every skill and agent reads. **Each
  ...
  | `graphify.md`                          | The code-intelligence protocol — graph-first for codebase questions, file reads as verification. Absence never blocks; only `setup` builds a graph, and `/vwf:doctor` §8 reports a missing CLI/graph/refresh-hook or a stale graph as a **degradation**                                                                                                                                                                                                      |
  ...
  | `format-check.md` + `blueprint-format` | The format-drift preflight: compare the repo's stamp to the shipped integer (**18**) and nudge `/vwf:setup`. Since vwf is user-scoped, this usage-time check is what reaches each repo                                                                                                                                                                                                                                                                       |
...
(currently **18**); the authoritative `N → N+1` migration deltas live in the
