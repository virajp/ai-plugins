...
(currently **21**); the authoritative `N → N+1` migration deltas live in the
...
first config bump since `11` to ship without a blueprint bump beside it. Format
`21` is the mirror case: a blueprint bump shipping without a config bump, since
it moves one config file and introduces no config key.
