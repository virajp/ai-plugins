...
- **`plugins:check`** — validates the authored tree. Ten rules: manifest
  ...
  **technology-free vwf** guard.
  ...
  subtler than it sounds. And the plugin-root rule caught a defect that had
  shipped in all four render trees for months: `${CLAUDE_PLUGIN_ROOT}` names
  only its *own* plugin, so `typescript` pointing at vwf's
  `delivery-pipeline.md` resolved to nothing at runtime. Both are in
  `.claude/skills/plugin-authoring/references/checks.md`, along with the eight
  rules that retired.
...
| `design-tools`   | `plugins/design-tools`   | **Draining — a manifest and nothing else.** Its three design-adapter skills merged into `vwf` (as `import-screens` / `import-design-system` / `import-conversations`, with one consolidated per-tool reference each under `vwf/assets/design-tools/`), and the plugin survives *only* to carry the Claude Design MCP server, which is a manifest feature no repo file can express. It retires once stackgen can generate that wiring on demand                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `devtools`       | `plugins/devtools`       | The **developer-machine toolchain** — the mise skill (the `.config/` three-file `MISE_ENV` split, tool/env placement, file-based tasks, node-gpg workaround) + `/devtools:scaffold`, `doppler` (**development** secrets only; no `secrets` plugin exists), and the repo-level gates: `dprint`, `eslint`, `gitleaks`, `grype`, `pre-commit`. **Its stack adapter retired in Wave C** and **its `docker` skill in Wave D** — `container-generic` is a stackgen bundle, the deploy artifact is stackgen's `deploy-target/container-image` pack, and the local stack is stackgen's harness contract. A **vwf dep**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `cicd`           | `plugins/cicd`           | A `/cicd:workflow` skill that resolves the repo's CI system (the per-project `cicd` key, else ask — **never** repo detection, **never** a silent default) and reads only that tool's `references/<tool>.md`; GitHub Actions is the one implemented today. Neutral rules live in SKILL.md (mise installs everything, both layouts, vwf's delivery-pipeline contract); adding a CI system is one reference file. **Independent — not a vwf dep** (vwf states the contract, `cicd` implements it)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `datastore`      | `plugins/datastore`      | **Capability** plugin — the neutral datastore contract (versioning, atomic multi-record writes, server time, the services-layer access rule, a deterministic local stack) plus the provider that needs no cloud: **Postgres**. Managed flavours come from the project's cloud plugin.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `identity`       | `plugins/identity`       | **Capability** plugin — the neutral identity contract (verification per route, the *claims carry status not roles* rule, revocation, the operator plane) plus the provider that belongs to no cloud: any **OIDC** issuer. Managed flavours come from the project's cloud plugin.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
describes throughout; the paired `config_format` (currently **15**) is described
...
directions: `14` shipped without a blueprint bump (it only closed the stack
menu) and `21` shipped without a config bump (it only moved one config file).
`22`/`15` shipped **together**, as `19`/`12` and `20`/`13` did — the config's
`template` pin and `ui:` key both depend on the platform vocabulary, so a repo
on one but not the other is a state neither migration expects. `23` then shipped
alone and purely additively: it lifts the blueprint-coverage exemption for the
`plugin` platform, retires no spelling, and needs no config key.
...
**binary** `mise` is a mandate `/vwf:doctor` blocks on.
...
**`design-tools` is no longer an adapter plugin.** Its three import skills are
vwf's own now — `/vwf:import-screens`, `/vwf:import-design-system`,
`/vwf:import-conversations` — invoked only when a project declares a `design:`
tool. vwf still names no tool: each skill **constructs** the path
`assets/design-tools/<tool>.md` from that config value and halts if no such
reference exists, so the three reference files are the only place a tool is
named, and they are the only three entries the technology-free guard's allowlist
gained. The plugin that remains is a carrier for the Claude Design MCP server
