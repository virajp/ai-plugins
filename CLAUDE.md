...
- **`plugins:marketplace`** — generates `.claude-plugin/marketplace.json` from
  the 13 `plugins/*/.claude-plugin/plugin.json` manifests, mapping `keywords` →
...
`plugins:marketplace` from the 13 plugin manifests, with `source` set to
...
Available plugin names: `vwf`, `typescript`, `flutter`, `devtools`, `cicd`,
`cloudflare`, `gcp`, `datastore`, `identity`, `observability`, `orchestration`,
`object-storage`, `stackgen`. Every one of them is authored here — no name on
