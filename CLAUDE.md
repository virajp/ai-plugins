...
- **`plugins:marketplace`** — generates `.claude-plugin/marketplace.json` from
  the 8 `plugins/*/.claude-plugin/plugin.json` manifests, mapping `keywords` →
...
`plugins:marketplace` from the 8 plugin manifests, with `source` set to
...
since `config_format` 14 there is no *other (describe)* escape — so with no
stack plugin installed, `/vwf:architecture` step 3b asks a question the user
cannot answer. `devtools`'s adapter retired in Wave C, which left vwf shipping
with no answer to its own menu. stackgen is the general-purpose stack plugin, so
it is the one that closes it. **Installing is not using**: stackgen acts only
when an axis is pinned, so a user adopting vwf first and choosing a stack later
pays nothing for having it present.
...
needs to be registered. `cicd` is **not** among them; install it by name when
you want pipelines generated. `mempalace` is not a name here at all — its memory
layer ships inside `vwf`. See the Dependencies section above.
