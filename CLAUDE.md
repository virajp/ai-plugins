...
- **`plugins:check`** — validates `templates/` **and** all four rendered
  ...
  rename), **`it.cmd()` target resolution** (every `it.cmd()` in a body *or* a
  skill/agent frontmatter description names a plugin in this marketplace and a
  skill it declares; an unqualified reference resolves against the plugin it is
  written in), cross-plugin skill-name uniqueness (skills share one flat
  namespace on OpenCode and Oh-My-Pi), the vwf design-adapter contract (all
  **three** import skills present and `invocation: both`), the **technology-free
  vwf** guard, relative links under `assets/examples/**`, and **strict-YAML
...
- `skills/` (workflow) — the `/vwf:` workflow skills (each
  ...
  | `readme`             | Scans the repo and writes/updates its README against the eight required sections. Absorbed from the retired `markdown` plugin; `invocation: both` **because `/vwf:docs-sync` routes a drifted README through it** — `setup` only names it in the chain it prints                                                                                                                   | —                                                                                                                                                                                                                    |
