...
A binary here could never be self-contained: Claude, Codex and Oh-My-Pi each
register a marketplace whose source is a real rendered directory, which the
agent re-reads *in place* on every later session, so the payload has to sit on
disk beside the executable rather than inside it. That made every non-npm
